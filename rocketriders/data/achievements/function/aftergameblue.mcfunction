...
execute if score $total_air_blocks var matches 0 if score $play_time match_data matches ..2400 run tag @a[x=0,predicate=custom:team/blue] add SimpleAndClean
...
execute if predicate game:portal_type/normal as @a[x=0,predicate=custom:team/yellow] store result score @s HasMissiles if items entity @s custom:player *[custom_data~{spawn_egg:{type:"missile"}}]
