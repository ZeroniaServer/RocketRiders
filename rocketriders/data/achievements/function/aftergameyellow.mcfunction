...
execute if score Yellow SwissCheese >= SwissCheese SwissCheese run tag @a[x=0,team=Yellow] add SwissCheese
scoreboard players reset SwissCheese SwissCheese

#Simple and Clean (part 1)
execute if score Yellow SwissCheese matches 0 if score @s gametime matches ..2400 run tag @a[x=0,team=Yellow] add SimpleAndClean
...
execute if score @s yellowsCount matches 1 if score @s bluesCount matches 2.. run tag @a[x=0,team=Yellow] add Loneliest

#Forsaken (part 1)
execute if score @s yellowsCount < @s origYCount run tag @a[x=0,team=Yellow] add Forsaken
...
tag @a[x=0,team=Yellow,scores={deaths=..0}] add Immortal

#I Swear I Had It (part 1) - necessary because players can die after game end
execute unless entity @s[tag=noPortal] as @a[x=0,team=Blue] store result score @s HasMissiles run clear @s #custom:missile 0
