...
execute if score Yellow SwissCheese >= SwissCheese SwissCheese run tag @a[predicate=custom:indimension,team=Yellow] add SwissCheese
scoreboard players reset SwissCheese SwissCheese

#Simple and Clean (part 1)
execute if score Yellow SwissCheese matches 0 if score @s gametime matches ..2400 run tag @a[predicate=custom:indimension,team=Yellow] add SimpleAndClean
...
execute as @e[predicate=custom:indimension,type=armor_stand,tag=machinacheck] at @s run function achievements:machinacheckyellow

#One is the Loneliest Number (part 1)
execute if score @s yellowsCount matches 1 if score @s bluesCount matches 2.. run tag @a[predicate=custom:indimension,team=Yellow] add Loneliest

#Forsaken (part 1)
execute if score @s yellowsCount < @s origYCount run tag @a[predicate=custom:indimension,team=Yellow] add Forsaken

#Ground Bound (part 1) - necessary because players can jump after game end
tag @a[predicate=custom:indimension,team=Yellow,scores={jumps=..0}] add GroundBound

#Immortal (part 1) - necessary because players can die after game end
tag @a[predicate=custom:indimension,team=Yellow,scores={deaths=..0}] add Immortal

#I Swear I Had It (part 1) - necessary because players can die after game end
execute unless entity @s[tag=noPortal] as @a[predicate=custom:indimension,team=Blue] store result score @s HasMissiles run clear @s #custom:missile 0
execute as @a[predicate=custom:indimension,team=Blue,scores={HasMissiles=1..}] at @s if entity @s[z=46,dz=28] run tag @s add HadIt
