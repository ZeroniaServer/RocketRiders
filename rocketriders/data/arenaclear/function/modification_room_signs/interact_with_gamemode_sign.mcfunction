...
# cycle options
execute unless predicate custom:player/is_pressing_sneak_key run scoreboard players add @e[x=0,type=armor_stand,tag=Selection,limit=1,scores={refreshsigns=..0}] SetGamemode 1
execute if predicate custom:player/is_pressing_sneak_key run scoreboard players remove @e[x=0,type=armor_stand,tag=Selection,limit=1,scores={refreshsigns=..0}] SetGamemode 1
