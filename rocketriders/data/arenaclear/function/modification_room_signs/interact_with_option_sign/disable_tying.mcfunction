...
execute unless predicate game:game_rules/disable_tying/on if entity @e[x=0,type=armor_stand,tag=Selection,limit=1,tag=!SignsRefreshed,tag=!ctfEnabled] run tellraw @s ["",{bold:true,color:"white",text:"Tie/Sudden Death"},{bold:true,color:"green",text:" Enabled:"},{color:"gray",italic:true,text:" Ties may occur if both portals are broken within 5 seconds of each other. This starts a Sudden Death period where the portals are restored and items are given twice as fast. Multiple ties may occur in one game, and the game only ends when one team's portal remains."}]
