...
execute if predicate game:game_rules/disable_tying/forced_on run return run tellraw @s {color:"dark_gray",italic:true,text:"Tying/Sudden Death is incompatible with this gamemode."}
execute if predicate game:game_rules/disable_tying/forced_off if entity @e[x=0,type=armor_stand,tag=Selection,limit=1,tag=ctfEnabled] run return run tellraw @s {color:"dark_gray",italic:true,text:"Overtime required in this gamemode."}
execute if predicate game:game_rules/disable_tying/forced_off run return run tellraw @s {color:"dark_gray",italic:true,text:"Tying/Sudden Death required in this gamemode."}
...
execute unless predicate game:game_rules/disable_tying/on if entity @e[x=0,type=armor_stand,tag=Selection,limit=1,tag=!SignsRefreshed,tag=!ctfEnabled] run tellraw @s ["",{bold:true,color:"white",text:"Tie/Sudden Death"},{bold:true,color:"green",text:" Enabled:"},{color:"gray",italic:true,text:" Ties may occur if both portals are broken within 5 seconds of each other. This starts a Sudden Death period where the portals are restored and items are given twice as fast. Multiple ties may occur in one match, and the game only ends when one team's portal remains."}]
...
execute if predicate game:game_rules/disable_tying/on if entity @e[x=0,type=armor_stand,tag=Selection,limit=1,tag=!SignsRefreshed,tag=!ctfEnabled] run tellraw @s ["",{bold:true,color:"white",text:"Tie/Sudden Death"},{bold:true,color:"red",text:" Disabled:"},{color:"gray",italic:true,text:" Only the first team to destroy the enemy portal can win the game."}]
