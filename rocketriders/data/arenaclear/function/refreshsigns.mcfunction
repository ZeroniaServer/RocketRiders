...
execute unless entity @s[tag=Repeat] run data modify block -70 192 79 front_text.messages[0] set value {"text":"Repeat Settings:","color":"black","click_event":{"action":"run_command","command":"/execute as @e[x=0,type=armor_stand,tag=Selection,limit=1] run function arenaclear:repeatincrement"}}
...
execute unless entity @s[tag=Repeat] run data modify block -70 192 79 front_text.messages[3] set value {"text":"(Click to adjust)","color":"gray","italic":true,"click_event":{"action":"run_command","command":"/execute if entity @e[x=0,type=armor_stand,tag=Selection,limit=1,tag=!SignsRefreshed] run playsound ui.button.click master @a[x=0] ~ ~ ~ 1 1"}}