...
execute unless predicate game:game_in_progress unless entity @e[x=0,type=armor_stand,tag=Selection,limit=1,tag=!rngNormal,tag=!rngHeavy,tag=!rngLightning] run tag @s add validClear
...
execute if predicate game:game_in_progress run tellraw @s {"text":"You can only run this when the game has ended","color":"red"}
execute unless predicate game:game_in_progress if entity @s[tag=!validClear] run tellraw @s {"text":"You must have at least one Missile enabled to start the game","color":"red"}
