...
execute as @e[predicate=custom:indimension,type=marker,tag=Platform,limit=1] run data modify entity @s Pos[0] set from storage rocketriders:canopypos x[0]
execute as @e[predicate=custom:indimension,type=marker,tag=Platform,limit=1] run data modify entity @s Pos[1] set from storage rocketriders:canopypos y[0]
execute as @e[predicate=custom:indimension,type=marker,tag=Platform,limit=1] run data modify entity @s Pos[2] set from storage rocketriders:canopypos z[0]

#Remove necessary blocks
execute as @e[predicate=custom:indimension,type=marker,tag=Platform,limit=1] at @s run fill ~-3 ~ ~-3 ~3 ~ ~3 air replace #minecraft:leaves
execute as @e[predicate=custom:indimension,type=marker,tag=Platform,limit=1] at @s run fill ~ ~ ~ ~ ~1 ~ air
kill @e[predicate=custom:indimension,type=marker,tag=Platform,limit=1]
