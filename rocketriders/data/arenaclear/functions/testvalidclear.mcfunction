...
execute if entity @e[type=armor_stand,tag=Selection,tag=Molerat,tag=!WasMolerat,tag=!MoleratOff] run tag @s add MoleratStop
execute if entity @e[type=armor_stand,tag=Selection,tag=!Molerat,tag=WasMolerat,tag=!MoleratOff] run tag @s add MoleratStop
...
execute if entity @s[tag=validClear] unless entity @s[tag=MoleratStop,tag=!moleratConfirm] at @s run playsound minecraft:ui.button.click master @a ~ ~ ~ 1 1
