...
# check range
$execute store success score $low var if predicate {condition:"minecraft:entity_properties",entity:"this",predicate:{movement:{horizontal_speed:{min:$(min),max:$(mean)}}}}
...
execute if score $diff var matches ..0100 run return run execute store result score $horizontal_speed var run scoreboard players operation $min var /= $ticks_per_second constant
...
return run function custom:__impl__/get_movement/horizontal_speed with storage rocketriders:main get_movement
