...
# check range
$execute store success score $low var if predicate {condition:"minecraft:entity_properties",entity:"this",predicate:{movement:{x:{min:$(min),max:$(mean)}}}}
...
execute if score $diff var matches ..0100 run return run execute store result score $x_velocity var run scoreboard players operation $min var /= $ticks_per_second constant
...
return run function custom:__impl__/get_movement/x_velocity with storage rocketriders:main get_movement
