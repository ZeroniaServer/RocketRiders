...
execute if predicate game:game_started unless predicate game:gamemode_components/neutral_utility_colors if entity @s[team=Yellow] if predicate custom:is_in_lava as @e[distance=..5,type=area_effect_cloud,tag=lavasplash_alone] on origin if entity @s[team=Blue] run advancement grant @s only achievements:rr_challenges/volcanic_hatred
execute if predicate game:game_started if predicate game:gamemode_components/neutral_utility_colors if predicate custom:on_blue_or_yellow_team if predicate custom:is_in_lava run tag @s add volcanic_hatred.victim
execute if predicate game:game_started if predicate game:gamemode_components/neutral_utility_colors if predicate custom:on_blue_or_yellow_team if predicate custom:is_in_lava as @e[distance=..5,type=area_effect_cloud,tag=lavasplash_alone] on origin if entity @s[tag=!volcanic_hatred.victim,predicate=custom:on_blue_or_yellow_team] run advancement grant @s only achievements:rr_challenges/volcanic_hatred
