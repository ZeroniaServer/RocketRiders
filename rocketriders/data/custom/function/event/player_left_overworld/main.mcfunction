...
execute if score $do_leave_message var matches 1 unless predicate game:phase/match if score $was_on_arena_team var matches 1 unless entity @e[limit=1,x=0,type=armor_stand,tag=Selection,tag=chaseEnabled] run tellraw @a[x=0] [{score:{name:"#blue",objective:"text.accent_color"}},[{score:{name:"#blue",objective:"text.main_color"}},{storage:"rocketriders:main",nbt:"player_left_overworld.name",interpret:true}]," left the blue team!"]
execute if score $do_leave_message var matches 1 unless predicate game:phase/match if score $was_on_arena_team var matches 1 if entity @e[limit=1,x=0,type=armor_stand,tag=Selection,tag=chaseEnabled] run tellraw @a[x=0] [{score:{name:"#blue",objective:"text.accent_color"}},[{score:{name:"#blue",objective:"text.main_color"}},{storage:"rocketriders:main",nbt:"player_left_overworld.name",interpret:true}]," left the match!"]
