...
execute if score $do_leave_message var matches 1 if predicate game:phase/match if score $was_on_arena_team var matches 3 run tellraw @a[x=0] ["",{color:"dark_gray",storage:"rocketriders:main",nbt:"player_left_overworld.name"},{color:"gray",text:" is no longer spectating the game!"}]
