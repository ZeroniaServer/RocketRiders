...
execute if data storage rocketriders:main explosion.modifiers{copy_name:true} if entity @s run loot replace block 0 184 -16 container.0 loot {pools:[{rolls:1,entries:[{type:"item",name:"stone",modifier:{type:"minecraft:set_name",entity:"this",name:{selector:"@s"}}}]}]}
