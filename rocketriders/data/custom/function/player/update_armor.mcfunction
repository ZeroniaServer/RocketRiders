...
execute if predicate custom:team/any_playing_team if predicate game:phase/match/play if predicate items:effects/elytra/on if items entity @s armor.chest *[custom_data~{id:"elytra",invisible:true}] unless predicate custom:entity/has_invisibility_effect run item modify block 0 184 -16 container.2 [{type:"minecraft:set_components",components:{"minecraft:equippable":{asset_id:"minecraft:elytra",damage_on_hurt:false,equip_sound:"minecraft:item.armor.equip_elytra",slot:"chest"}}},{type:"minecraft:set_custom_data",tag:{invisible:false}}]
# Invisibility
