...
execute if predicate custom:team/lobby if predicate rr:server_mode/cubekrowd_voting if predicate game:phase/staging unless entity @e[limit=1,x=0,type=armor_stand,tag=Selection,tag=EditedSettings] unless score @s VoteNum matches 1.. unless items block 0 184 -16 container.0 *[custom_data~{id:"voting_ballot"},!custom_data~{voting_ballot:{used:true}}] run loot replace block 0 184 -16 container.0 loot servermode:voting_ballot
execute if predicate custom:team/lobby if predicate rr:server_mode/cubekrowd_voting if predicate game:phase/staging unless entity @e[limit=1,x=0,type=armor_stand,tag=Selection,tag=EditedSettings] if score @s VoteNum matches 1.. unless items block 0 184 -16 container.0 *[custom_data~{id:"voting_ballot"},custom_data~{voting_ballot:{used:true}}] run loot replace block 0 184 -16 container.0 loot servermode:voting_ballot_used
