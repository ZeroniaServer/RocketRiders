...
execute if score @s entity.canopy.movement_cooldown matches 1.. on origin if predicate custom:player/is_nova_attached run return run function entities:type/canopy/tick/trigger_nova_attach

# force_mount tag is used by the CK plugin to prevent dismounting
execute if score @s entity.canopy.movement_cooldown matches ..39 if predicate game:phase/match/play on origin run tag @s add canopy.player
...
execute if score @s entity.canopy.movement_cooldown matches ..39 if predicate game:phase/match/play on origin run tag @s[type=player,predicate=custom:player/is_alive] add force_mount
