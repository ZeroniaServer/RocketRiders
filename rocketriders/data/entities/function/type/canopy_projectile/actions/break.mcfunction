...
execute on vehicle if entity @s[predicate=entities:type/canopy_projectile/body] positioned as @s run return run function entities:type/canopy_projectile/actions/break

# Particles
execute at @s run particle minecraft:item{item:{id:"minecraft:ender_pearl"}} ~ ~ ~ 0 0 0 0.05 5 force @a[x=0,tag=!hideParticles,predicate=custom:in_arena]
execute at @s run particle minecraft:dust_color_transition{from_color:[1,0,1],to_color:[0,1,0],scale:1} ~ ~ ~ 0.1 0.2 0.1 0 3 force @a[x=0,tag=!hideParticles,predicate=custom:in_arena]
