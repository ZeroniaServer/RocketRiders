...
execute rotated as @s positioned ^ ^1.5 ^ run data modify entity @e[limit=1,distance=..0.01,type=fireball,tag=cluster_fireball.child,tag=cluster_fireball.new] Owner set from storage rocketriders:main cluster_fireball.owner
execute rotated as @s run summon fireball ^ ^-1.5 ^ {Tags:["ClusterFireball","cluster_fireball.child","cluster_fireball.new"],ExplosionPower:1,Motion:[0.0d,0.0d,0.0d],Item:{id:"minecraft:blaze_powder"}}
