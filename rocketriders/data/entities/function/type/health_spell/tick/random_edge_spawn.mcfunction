...
$execute rotated $(yaw) $(pitch) positioned ^ ^ ^$(radius) as @e[distance=..10,type=area_effect_cloud,tag=!health_spell.this,predicate=entities:type/health_spell] if function entities:type/health_spell/tick/check_too_close run return 0
