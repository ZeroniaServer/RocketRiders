...
execute on vehicle if entity @s[predicate=entities:type/nova_rocket/body] positioned as @s run return run function entities:type/nova_rocket/actions/break

# Particles
execute at @s run particle minecraft:item{item:{id:"minecraft:firework_rocket"}} ~ ~ ~ 0 0 0 0.05 5 force @a[x=0,tag=!hideParticles,predicate=custom:in_arena]
execute at @s run particle minecraft:smoke ~ ~ ~ 0 0 0 0.05 10 force @a[x=0,tag=!hideParticles,predicate=custom:in_arena]
