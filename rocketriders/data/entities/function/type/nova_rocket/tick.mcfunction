...
execute on vehicle positioned as @s if predicate custom:near_or_above_roof run return run function entities:type/nova_rocket/actions/break_with_reason {message:"Nova Rocket failed to deploy; it was too close to the roof"}
execute on vehicle positioned as @s if predicate custom:in_void unless predicate custom:entity/is_moving_upwards run return run function entities:type/nova_rocket/actions/break_with_reason {message:"Nova Rocket failed to deploy; it was too close to the void"}
execute on vehicle positioned as @s if predicate custom:location/touching_or_beyond_world_border run return run function entities:type/nova_rocket/actions/break_with_reason {message:"Nova Rocket failed to deploy; it was too close to the world border"}
execute on vehicle positioned as @s unless predicate custom:in_arena run return run function entities:type/nova_rocket/actions/break_with_reason {message:"Nova Rocket failed to deploy; it left the arena"}

# Early impact
execute unless predicate custom:entity/has_vehicle if function custom:projectile_motion_step positioned as @s run return run function entities:type/nova_rocket/actions/explode
