...
execute store success score $flip_horizontal var positioned 0.0 0.0 0.0 rotated ~45 0 positioned ^ ^ ^-0.5 align xz facing -0.5 0 -0.5 rotated ~-45 0 unless block ^ ^ ^0.6 #custom:non_solid
execute store success score $towards_down var positioned ^ ^ ^0.1 positioned ~-0.5 ~ ~-0.5 if entity @s[dy=0]
execute if score $towards_down var matches 0 store success score $flip_vertical var unless block ~ ~1.1 ~ #custom:non_solid
execute if score $towards_down var matches 1 store success score $flip_vertical var unless block ~ ~-0.1 ~ #custom:non_solid

execute positioned 0.0 0.0 0.0 summon marker run function entities:type/wind_spell/tick/get_motion_vector with storage rocketriders:main wind_spell
