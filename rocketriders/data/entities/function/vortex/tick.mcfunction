...
## Achievements
execute if entity @s[tag=vortex.feathered] if entity @e[x=0,type=armor_stand,tag=Selection,limit=1,scores={servermode=-1..0},predicate=!rr:realms] run advancement grant @a[distance=..3,predicate=custom:on_blue_or_yellow_team] only achievements:rr_challenges/zzzzzzhuh
