...
execute if score $was_drifting var matches 0 on passengers if entity @s[predicate=entities:vortex/brain,tag=!vortex.feathered] on vehicle run data merge entity @s {teleport_duration:2,item:{id:"minecraft:ender_eye"},start_interpolation:0,interpolation_duration:5,transformation:{scale:[1.2,1.2,1.2]}}
...
execute on passengers run tag @s[predicate=entities:vortex/brain] add vortex.is_drifting
