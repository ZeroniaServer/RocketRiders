...
execute as @a[x=0,predicate=custom:team/any_playing_team,predicate=custom:player/has_broken_trident_in_inventory] at @s run function everytick:trident_item_break
execute as @a[x=0,predicate=custom:team/any_playing_team,predicate=custom:player/has_trident_in_inventory] if items entity @s weapon.* trident at @s run function everytick:trident_riptide
