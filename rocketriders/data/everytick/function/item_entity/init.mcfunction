...
execute if predicate rr:drop_main_item_to_leave_match if predicate game:game_running if items entity @s contents *[custom_data~{main_item:true}] on origin if predicate custom:team/any_playing_team run function everytick:item_entity/leave_mid_match_message
