...
execute if predicate game:phase/match/play unless predicate custom:item_entity_contains_any_arrow positioned ~-1.25 ~-0.499999 ~-1.25 as @e[type=player,dx=1.5,dy=0.249998,dz=1.5,predicate=custom:team/any_playing_team] store success score $is_near_someone var run tag @s add item_entity.target_candidate
execute if predicate game:phase/match/play if predicate custom:item_entity_contains_any_arrow positioned ~-1.25 ~-0.499999 ~-1.25 as @e[type=player,dx=1.5,dy=0.249998,dz=1.5,predicate=custom:can_pick_up_arrows,predicate=custom:team/any_playing_team] store success score $is_near_someone var run tag @s add item_entity.target_candidate
