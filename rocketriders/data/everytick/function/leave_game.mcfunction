...
execute unless predicate game:phase/match/over if entity @s[tag=EditedSettings,tag=!SMSwitch] run tag @a[x=0,scores={LeaveGame=1..,LeaveMidgame=0},tag=!informMeLate] add informMe
execute unless predicate game:phase/match/over if entity @s[tag=EditedSettings,tag=!SMSwitch] run scoreboard players add @a[x=0,scores={informMeLate=1..120}] informMeLate 1
execute unless predicate game:phase/match/over if entity @s[tag=EditedSettings,tag=!SMSwitch] run scoreboard players add @a[x=0,tag=informMeLate] informMeLate 1
execute unless predicate game:phase/match/over if entity @s[tag=EditedSettings,tag=!SMSwitch] run tag @a[x=0,scores={informMeLate=1..}] remove informMeLate
execute unless predicate game:phase/match/over if entity @s[tag=EditedSettings,tag=!SMSwitch] run tag @a[x=0,scores={informMeLate=120..}] add informMe
execute unless predicate game:phase/match/over if entity @s[tag=EditedSettings,tag=!SMSwitch] run scoreboard players reset @a[x=0,tag=informMe] informMeLate
execute unless entity @s[predicate=!game:phase/match/over,tag=EditedSettings,tag=!SMSwitch] run tag @a[x=0] remove informMeLate
execute unless entity @s[predicate=!game:phase/match/over,tag=EditedSettings,tag=!SMSwitch] run scoreboard players reset @a[x=0] informMeLate
