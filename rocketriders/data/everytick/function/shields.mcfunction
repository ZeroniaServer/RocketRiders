...
execute if entity @e[x=0,type=armor_stand,tag=Selection,tag=!customShield] as @e[x=0,type=snowball,tag=BlueShield] at @s if score $dust CmdData matches 1 run particle minecraft:dust{color:[0,0,1],scale:1} ~ ~ ~ 0 0 0 0.1 10 force @a[x=0,tag=!hideParticles,predicate=custom:belowroof]
...
execute if entity @e[x=0,type=armor_stand,tag=Selection,tag=!customShield] as @e[x=0,type=marker,tag=PlaceBlueShield,scores={shieldplacement=3}] at @s run particle minecraft:block{block_state:"minecraft:blue_stained_glass"} ~ ~ ~ 1 1 0 0.1 50
