...
execute as @e[x=0,type=area_effect_cloud,tag=splash_alone,tag=!splashMarked,predicate=custom:near_any_spawn_zone] run function custom:entity/kill_with_reason {message:"Splash hit the ground too close to a spawnpoint"}
...
execute at @e[x=0,type=area_effect_cloud,tag=splash_alone,tag=!splashMarked] unless block ~1 ~ ~ #custom:non_solid unless block ~-1 ~ ~ #custom:non_solid unless block ~ ~ ~1 #custom:non_solid unless block ~ ~ ~-1 #custom:non_solid if block ~ ~-1 ~ #custom:air run fill ~ ~ ~ ~ ~-1 ~ water[level=8] replace #custom:air
