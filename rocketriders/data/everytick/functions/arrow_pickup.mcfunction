...
execute as @a[x=0,team=!Spectator] at @s as @e[x=0,type=arrow,scores={arrowtime=10..},nbt={inGround:1b},tag=!infinity,limit=1,distance=..2,sort=nearest] unless data entity @s CustomPotionEffects run tag @s add pickup
...
#Give arrow
execute as @a[x=0,team=!Spectator,tag=!fullOffhand] at @s as @e[x=0,type=arrow,tag=pickup,tag=!given,nbt={inGround:1b},tag=!infinity,limit=1,distance=..2,sort=nearest] unless data entity @s CustomPotionEffects at @s run function everytick:arrow_replace
execute if entity @s[tag=!GameStarted] as @e[x=0,type=arrow,tag=pickup,tag=!given,nbt={inGround:1b},tag=!infinity] unless data entity @s CustomPotionEffects at @s run give @p[team=!Spectator,distance=..2,tag=fullOffhand] arrow{display:{Name:'{"text":"Arrow","color":"light_purple","italic":true,"bold":true}'},Lobby:1b}
execute if entity @s[tag=GameStarted] as @e[x=0,type=arrow,tag=pickup,tag=!given,nbt={inGround:1b},tag=!infinity] unless data entity @s CustomPotionEffects at @s run give @p[team=!Spectator,distance=..2,tag=fullOffhand] arrow{display:{Name:'{"text":"Arrow","color":"light_purple","italic":true,"bold":true}'}}
