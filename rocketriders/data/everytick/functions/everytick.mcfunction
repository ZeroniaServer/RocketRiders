...
execute as @e[type=armor_stand,tag=Selection,tag=ServerModeVoting] if entity @a[predicate=custom:indimension] run function servermode:loop
scoreboard players reset @e[type=armor_stand,tag=Selection,tag=!ServerModeVoting] VoteServerMode
execute unless entity @a[predicate=custom:indimension,team=Lobby] run scoreboard players reset @e[type=armor_stand,tag=Selection,tag=ServerModeVoting] VoteServerMode
execute as @e[type=armor_stand,tag=Selection,scores={servermode=1..}] unless entity @a[predicate=custom:indimension] run function game:forcestop
execute as @e[type=armor_stand,tag=Selection,scores={servermode=0},tag=SMCustom] unless entity @a[predicate=custom:indimension] run scoreboard players set @s RepeatSettings 0
execute as @e[type=armor_stand,tag=Selection,scores={servermode=0},tag=SMCustom] unless entity @a[predicate=custom:indimension] run function game:forcestop
execute as @e[type=armor_stand,tag=Selection,scores={servermode=0},tag=SMCustom] unless entity @a[predicate=custom:indimension] run function arenaclear:globaldefaults
execute unless entity @e[type=armor_stand,tag=Selection,tag=ServerModeVoting] run scoreboard players reset @a[predicate=custom:indimension] VoteServerMode
...
execute as @e[type=armor_stand,tag=Selection] in minecraft:overworld run function everytick:general_settings_or_hotfixes
...
execute as @e[type=armor_stand,tag=Selection,tag=!Sonar] run effect give @a[predicate=custom:indimension,team=Spectator] night_vision infinite 100 true
execute as @e[type=armor_stand,tag=Selection,tag=!SMActive] run effect give @a[predicate=custom:indimension] saturation infinite 0 true
execute as @e[type=armor_stand,tag=Selection,tag=SMActive] run effect clear @a[predicate=custom:indimension] saturation
...
# execute if entity @e[type=marker,tag=ArenaClearChecker] as @a[predicate=custom:indimension] run function everytick:stopsounds
