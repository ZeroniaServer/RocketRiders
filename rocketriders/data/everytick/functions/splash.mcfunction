...
tag @e[type=minecraft:potion,nbt={Item:{id:"minecraft:lingering_potion",Count:1b,tag:{Potion:"minecraft:water",CustomPotionColor:3237342,CustomPotionEffects:[{Duration:1,Id:23b,Amplifier:0b,ShowParticles:0b}]}}},tag=!splash] add splash
...
execute as @e[tag=splashclear] at @s if entity @e[tag=splash_alone,limit=1,sort=nearest,distance=..1] store result score @s splashOwnerUUID run data get entity @e[tag=splash_alone,limit=1,sort=nearest,distance=..1] Owner[0]
kill @e[tag=splash_alone]
execute as @e[tag=splashclear,nbt={Age:300}] at @s unless block ~ ~ ~ water run scoreboard players reset @s splashOwnerUUID
