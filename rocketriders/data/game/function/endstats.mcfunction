...
execute if predicate game:outcome/yellow_won_only run tellraw @a[x=0] [{bold:true,score:{name:"#yellow",objective:"text.main_color"}},"",{score:{name:"#yellow",objective:"text.team_name"}}," Team Wins!"]
execute if predicate game:outcome/blue_won_only run tellraw @a[x=0] [{bold:true,score:{name:"#blue",objective:"text.main_color"}},"",{score:{name:"#blue",objective:"text.team_name"}}," Team Wins!"]
# TODO: this should be driven by a match component instead, probably, but this is a quick and dirty fix
execute if predicate game:outcome/both_won if entity @s[tag=!duelEnabled,tag=!sandboxEnabled] run tellraw @a[x=0] {bold:true,color:"green",text:"Everyone Wins!"}
execute if predicate game:outcome/both_won if entity @s[tag=duelEnabled] run tellraw @a[x=0] {bold:true,color:"gray",text:"It's a Draw!"}
