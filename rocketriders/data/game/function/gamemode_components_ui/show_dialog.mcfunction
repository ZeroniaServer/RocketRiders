...
execute if score $config_override.disable_hotbar_limit gamemode_components matches -1 run data modify storage rocketriders:main gamemode_components.dialog.inputs append value {type:"minecraft:single_option",key:"",label:{color:"gray",text:"🔒 config_override.disable_hotbar_limit"},width:300,options:[{id:"0",display:"off"}]}
