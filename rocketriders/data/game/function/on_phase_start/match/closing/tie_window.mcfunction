...
execute if predicate game:phase/match/closing/tie_window if predicate game:game_rules/tie_window_length/non_zero if predicate game:yellow_portal_revealed if predicate game:blue_portal_revealed unless entity @s[tag=BothWon] run scoreboard players set $skip_tie_window var 0
