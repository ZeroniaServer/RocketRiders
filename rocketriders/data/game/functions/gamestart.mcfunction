...
execute as @s[tag=BlueFull,tag=EditedSettings] as @e[tag=bluejoinpad,tag=CancelJoin,type=area_effect_cloud] at @s run title @a[distance=..1,team=Lobby,tag=!tryJoinBlue] title ["",{"text":"Cannot Join Team!","color":"red","bold":true}]
...
execute as @e[tag=specjoinpad,tag=!CancelJoin,type=area_effect_cloud] at @s run tag @a[team=!Spectator,distance=..1,limit=1,sort=random] add JoinSpec
