...
execute unless score $reloaded CmdData matches 1..100 unless entity @e[type=armor_stand,tag=Selection,tag=NoModesInstalled] unless entity @e[type=armor_stand,tag=gamemodeAS] run tellraw @a [{"text":"Error: No gamemodes installed! Please install at least one gamemode to play Rocket Riders.","color":"red"}]
