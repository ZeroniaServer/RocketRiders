...
execute unless score $reloaded CmdData matches 1..20 unless entity @e[type=armor_stand,tag=Selection,tag=NoModesInstalled] unless entity @e[type=armor_stand,tag=gamemodeAS] run tellraw @a [{"text":"Error: No gamemodes installed! Please install at least one gamemode to play Rocket Riders.","color":"red"}]
execute unless score $reloaded CmdData matches 1..20 unless entity @e[type=armor_stand,tag=gamemodeAS] run tag @e[type=armor_stand,tag=Selection] add NoModesInstalled
execute unless score $reloaded CmdData matches 1..20 if entity @e[type=armor_stand,tag=gamemodeAS] run tag @e[type=armor_stand,tag=Selection] remove NoModesInstalled
execute if score $reloaded CmdData matches 1..20 run tag @e[type=armor_stand,tag=Selection] remove NoModesInstalled
