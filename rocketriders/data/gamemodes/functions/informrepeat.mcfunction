...
execute if entity @s[scores={RepeatSettings=2..}] run tellraw @a[tag=informMe] ["",{"text":"| ","color":"dark_gray","bold":true},{"text":"(Repeating settings for ","color":"gray","italic":true},{"score":{"name":"@e[type=armor_stand,tag=Selection,limit=1]","objective":"RepeatSettings"},"color":"white","italic":true},{"text":" games!)","color":"gray","italic":true}]
