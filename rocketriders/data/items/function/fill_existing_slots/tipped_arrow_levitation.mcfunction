...
loot give @s loot {pools:[{rolls:1,entries:[{type:"minecraft:loot_table",value:"items:misc/tipped_arrow_levitation"}],functions:[{function:"minecraft:set_count",count:{type:"minecraft:score",target:{type:"minecraft:fixed",name:"$existing_slots_remainder"},score:"var"}}]}]}
scoreboard players operation $count var -= $existing_slots_remainder var
execute if score $count var matches 1.. run function items:fill_existing_slots/tipped_arrow_levitation
