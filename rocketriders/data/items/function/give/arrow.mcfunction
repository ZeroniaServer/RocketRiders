...
function items:fill_existing_slots/arrow
...
execute if score $offhand var matches 1 if score $count var matches ..64 run return run loot replace entity @s weapon.offhand loot {pools:[{rolls:1,entries:[{type:"minecraft:loot_table",value:"items:misc/arrow"}],functions:[{function:"minecraft:set_count",count:{type:"minecraft:score",target:{type:"minecraft:fixed",name:"$count"},score:"var"}}]}]}

execute if score $offhand var matches 1 run loot replace entity @s weapon.offhand loot {pools:[{rolls:1,entries:[{type:"minecraft:loot_table",value:"items:misc/arrow"}],functions:[{function:"minecraft:set_count",count:64}]}]}
execute if score $offhand var matches 1 run scoreboard players remove $count var 64

# Then dump any remaining items
loot give @s loot {pools:[{rolls:{type:"minecraft:score",target:{type:"minecraft:fixed",name:"$count"},score:"var"},entries:[{type:"minecraft:loot_table",value:"items:misc/arrow"}]}]}
