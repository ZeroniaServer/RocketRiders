...
execute if score $increase_item_limit.canopy match_components matches 1.. run scoreboard players operation $item_limit var += $increase_item_limit.canopy match_components
...
execute as @e[x=0,type=item] if items entity @s contents *[custom_data~{id:"canopy"},!custom_data~{droppable:true}] if function custom:match_origin run function items:give/__return_item
...
## Fail
execute if score $give_item var matches 0 if predicate game:game_rules/show_debug_logs/on run function custom:log {message:["(items:give/canopy) Failed to give Canopy to ",{selector:"@s"}]}
...
execute if predicate game:game_rules/show_debug_logs/on run function custom:log {message:["(items:give/canopy) Gave Canopy x",{score:{name:"$final_batch_size",objective:"var"}}," to ",{selector:"@s"}]}
