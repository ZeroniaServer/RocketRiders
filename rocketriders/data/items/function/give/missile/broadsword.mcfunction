...
execute as @e[x=0,type=item] if items entity @s contents *[custom_data~{id:"missile/broadsword"},!custom_data~{droppable:true}] if function custom:match_origin run function items:give/__return_item
...
## Fail
execute if score $give_item var matches 0 if predicate game:game_rules/show_debug_logs/on run function custom:log {message:["(items:give/missile/broadsword) Failed to give Broadsword to ",{selector:"@s"}]}
...
execute if predicate game:game_rules/show_debug_logs/on run function custom:log {message:["(items:give/missile/broadsword) Gave Broadsword x",{score:{name:"$final_batch_size",objective:"var"}}," to ",{selector:"@s"}]}
