...
execute as @e[x=0,type=item] if items entity @s contents *[custom_data~{id:"missile/catapult"},!custom_data~{droppable:true}] if function custom:match_origin run function items:give/__return_item
...
execute if predicate game:game_rules/show_debug_logs/on run function custom:log {message:["(items:give/missile/catapult) Gave Catapult x",{score:{name:"$final_batch_size",objective:"var"}}," to ",{selector:"@s"}]}
