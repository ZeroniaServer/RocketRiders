...
execute as @e[x=0,type=item] if items entity @s contents *[custom_data~{id:"spell_book"},!custom_data~{droppable:true}] if function custom:match_origin run function items:give/__return_item
...
## Fail
execute if score $give_item var matches 0 if predicate game:game_rules/show_debug_logs/on run function custom:log {message:["(items:give/spell_book) Failed to give Spell Book to ",{selector:"@s"}]}
...
execute if predicate game:game_rules/show_debug_logs/on run function custom:log {message:["(items:give/spell_book) Gave Spell Book x",{score:{name:"$final_batch_size",objective:"var"}}," to ",{selector:"@s"}]}
