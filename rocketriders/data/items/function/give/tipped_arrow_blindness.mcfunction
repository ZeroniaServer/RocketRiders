...
## Fail
execute if score $give_item var matches 0 if predicate game:game_rules/show_debug_logs/on run function custom:log {message:["(items:give/tipped_arrow_blindness) Failed to give Blindness Arrow to ",{selector:"@s"}]}
...
execute if predicate game:game_rules/show_debug_logs/on run function custom:log {message:["(items:give/tipped_arrow_blindness) Gave Blindness Arrow x",{score:{name:"$final_batch_size",objective:"var"}}," to ",{selector:"@s"}]}
