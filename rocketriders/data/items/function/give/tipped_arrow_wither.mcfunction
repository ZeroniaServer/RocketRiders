...
## Fail
execute if score $give_item var matches 0 if predicate game:game_rules/show_debug_logs/on run function custom:log {message:["(items:give/tipped_arrow_wither) Failed to give Wither Arrow to ",{selector:"@s"}]}
...
execute if predicate game:game_rules/show_debug_logs/on run function custom:log {message:["(items:give/tipped_arrow_wither) Gave Wither Arrow x",{score:{name:"$final_batch_size",objective:"var"}}," to ",{selector:"@s"}]}
