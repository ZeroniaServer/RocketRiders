###################################################
## Conditionally gives the executor Vortex items ##
###################################################

## Game tracking
tag @e[limit=1,x=0,type=armor_stand,tag=Selection,tag=tetrisTime] add givenVortex

## Get limit
scoreboard players set $item_limit var 1
execute if score $increase_item_limit.vortex match_components matches 1.. run scoreboard players operation $item_limit var += $increase_item_limit.vortex match_components
...
execute as @e[x=0,type=item] if items entity @s contents *[custom_data~{id:"vortex"},!custom_data~{droppable:true}] if function custom:match_origin run function items:give/__return_item
tag @s remove matchOrigin
execute store result score $inventory_count var run clear @s *[custom_data~{id:"vortex"}] 0
...
# limit reached
execute if score $give_item var matches 1 if score $final_batch_size var matches 0 if score $item_limit var matches 1 run title @s actionbar {color:"aqua",text:"Vortex already obtained."}
execute if score $give_item var matches 1 if score $final_batch_size var matches 0 if score $item_limit var matches 2.. run title @s actionbar {color:"aqua",text:"Maximum Vortices already obtained."}
...
## Fail
execute if score $give_item var matches 0 if predicate game:game_rules/show_debug_logs/on run function custom:log {message:["(items:give/givenova) Failed to give Vortex to ",{selector:"@s"}]}
...
execute if predicate game:game_rules/show_debug_logs/on run function custom:log {message:["(items:give/givenova) Gave Vortex x",{score:{name:"$final_batch_size",objective:"var"}}," to ",{selector:"@s"}]}
