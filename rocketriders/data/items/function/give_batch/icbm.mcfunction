...
execute if score $base_item_limit.icbm match_components matches 1.. run scoreboard players operation $item_limit var = $base_item_limit.icbm match_components
...
execute as @e[x=0,type=item] if items entity @s contents *[custom_data~{id:"icbm"},!custom_data~{droppable:true}] if function custom:match_origin run function items:give_batch/__return_item
tag @s remove matchOrigin
execute store result score $inventory_count var if items entity @s custom:player *[custom_data~{id:"icbm"}]

## Get batch size
scoreboard players set $intended_batch_size var 1
execute if score $item_batch_size.icbm match_components matches 1.. run scoreboard players operation $intended_batch_size var = $item_batch_size.icbm match_components
...
# limit reached
execute if score $give_item var matches 1 if score $final_batch_size var matches 0 if score $item_limit var matches 1 run title @s actionbar {color:"red",text:"I.C.B.M. already obtained"}
execute if score $give_item var matches 1 if score $final_batch_size var matches 0 if score $item_limit var matches 2.. run title @s actionbar {color:"red",text:"Maximum I.C.B.M.s already obtained"}
...
## Fail
execute if score $give_item var matches 0 if predicate game:game_rules/show_debug_logs/on run function custom:log {message:["(items:give_batch/icbm) Failed to give I.C.B.M. to ",{selector:"@s"}]}
...
execute if predicate game:game_rules/show_debug_logs/on run function custom:log {message:["(items:give_batch/icbm) Gave I.C.B.M. x",{score:{name:"$final_batch_size",objective:"var"}}," to ",{selector:"@s"}]}
