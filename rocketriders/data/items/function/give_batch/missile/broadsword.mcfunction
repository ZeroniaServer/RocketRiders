#######################################################
## Conditionally gives the executor Broadsword items ##
#######################################################
...
execute if score $increase_item_limit.missile/broadsword match_components matches 1.. run scoreboard players operation $item_limit var += $increase_item_limit.missile/broadsword match_components
...
execute if score $item_batch_size.missile/broadsword match_components matches 1.. run scoreboard players operation $intended_batch_size var = $item_batch_size.missile/broadsword match_components
...
# limit reached
execute if score $give_item var matches 1 if score $final_batch_size var matches 0 if score $item_limit var matches 1 run title @s actionbar {color:"aqua",text:"Broadsword already obtained"}
execute if score $give_item var matches 1 if score $final_batch_size var matches 0 if score $item_limit var matches 2.. run title @s actionbar {color:"aqua",text:"Maximum Broadswords already obtained"}
...
## Fail
execute if score $give_item var matches 0 if predicate game:game_rules/show_debug_logs/on run function custom:log {message:["(items:give_batch/missile/broadsword) Failed to give Broadsword to ",{selector:"@s"}]}
...
## Success
execute if score $final_batch_size var matches 1 run title @s actionbar {color:"aqua",text:"Broadsword obtained"}
execute if score $final_batch_size var matches 2.. run title @s actionbar {color:"aqua",text:"Broadswords obtained"}
execute if predicate game:game_rules/show_debug_logs/on run function custom:log {message:["(items:give_batch/missile/broadsword) Gave Broadsword x",{score:{name:"$final_batch_size",objective:"var"}}," to ",{selector:"@s"}]}
execute store result storage rocketriders:main items.count int 1 run scoreboard players get $final_batch_size var
function items:give_count/missile/broadsword with storage rocketriders:main items
