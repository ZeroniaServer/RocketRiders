...
## Game tracking
tag @e[x=0,type=armor_stand,tag=Selection,limit=1,tag=tetrisTime] add givenCita
#tag @e[x=0,type=armor_stand,tag=Bot] add HasCita

## Get limit
scoreboard players set $item_limit var 1
execute if score $base_item_limit.missile/citadel match_components matches 1.. run scoreboard players operation $item_limit var = $base_item_limit.missile/citadel match_components
...
execute store result score $inventory_count var if items entity @s custom:player *[custom_data~{id:"missile/citadel"}]

## Get batch size
scoreboard players set $intended_batch_size var 1
execute if score $item_batch_size.missile/citadel match_components matches 1.. run scoreboard players operation $intended_batch_size var = $item_batch_size.missile/citadel match_components
...
# limit reached
execute if score $give_item var matches 1 if score $final_batch_size var matches 0 if score $item_limit var matches 1 run title @s actionbar {color:"red",text:"Citadel already obtained"}
execute if score $give_item var matches 1 if score $final_batch_size var matches 0 if score $item_limit var matches 2.. run title @s actionbar {color:"red",text:"Maximum Citadels already obtained"}
