#######################################################
## Conditionally gives the executor Spell Book items ##
#######################################################

## Game tracking
tag @e[limit=1,x=0,type=armor_stand,tag=Selection,tag=tetrisTime] add givenSpellBook

## Get limit
scoreboard players set $item_limit var 1
execute if score $base_item_limit.spell_book match_components matches 1.. run scoreboard players operation $item_limit var = $base_item_limit.spell_book match_components
...
execute as @e[x=0,type=item] if items entity @s contents *[custom_data~{id:"spell_book"},!custom_data~{droppable:true}] if function custom:match_origin run function items:give_batch/__return_item
tag @s remove matchOrigin
execute store result score $inventory_count var if items entity @s custom:player *[custom_data~{id:"spell_book"}]

## Get batch size
scoreboard players set $intended_batch_size var 1
execute if score $item_batch_size.spell_book match_components matches 1.. run scoreboard players operation $intended_batch_size var = $item_batch_size.spell_book match_components
...
## Fail
execute if score $give_item var matches 0 if predicate game:game_rules/show_debug_logs/on run function custom:log {message:["(items:give_batch/spell_book) Failed to give Spell Book to ",{selector:"@s"}]}
...
## Success
execute if score $final_batch_size var matches 1 run title @s actionbar {color:"aqua",text:"Spell Book obtained"}
execute if score $final_batch_size var matches 2.. run title @s actionbar {color:"aqua",text:"Spell Books obtained"}
execute if predicate game:game_rules/show_debug_logs/on run function custom:log {message:["(items:give_batch/spell_book) Gave Spell Book x",{score:{name:"$final_batch_size",objective:"var"}}," to ",{selector:"@s"}]}
execute store result storage rocketriders:main items.count int 1 run scoreboard players get $final_batch_size var
function items:give_count/spell_book with storage rocketriders:main items
