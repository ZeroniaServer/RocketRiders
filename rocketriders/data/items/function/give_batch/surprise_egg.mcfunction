#########################################################
## Conditionally gives the executor Surprise Egg items ##
#########################################################
...
execute if score $base_item_limit.surprise_egg match_components matches 1.. run scoreboard players operation $item_limit var = $base_item_limit.surprise_egg match_components
...
execute as @e[x=0,type=item] if items entity @s contents *[custom_data~{id:"surprise_egg"},!custom_data~{droppable:true}] if function custom:match_origin run function items:give_batch/__return_item
tag @s remove matchOrigin
execute store result score $inventory_count var if items entity @s custom:player *[custom_data~{id:"surprise_egg"}]

## Get batch size
scoreboard players set $intended_batch_size var 1
execute if score $item_batch_size.surprise_egg match_components matches 1.. run scoreboard players operation $intended_batch_size var = $item_batch_size.surprise_egg match_components
...
## Fail
execute if score $give_item var matches 0 if predicate game:game_rules/show_debug_logs/on run function custom:log {message:["(items:give_batch/surprise_egg) Failed to give Surprise Egg to ",{selector:"@s"}]}
...
## Success
execute if score $final_batch_size var matches 1 run title @s actionbar {color:"aqua",text:"Surprise Egg obtained"}
execute if score $final_batch_size var matches 2.. run title @s actionbar {color:"aqua",text:"Surprise Eggs obtained"}
execute if predicate game:game_rules/show_debug_logs/on run function custom:log {message:["(items:give_batch/surprise_egg) Gave Surprise Egg x",{score:{name:"$final_batch_size",objective:"var"}}," to ",{selector:"@s"}]}
execute store result storage rocketriders:main items.count int 1 run scoreboard players get $final_batch_size var
function items:give_count/surprise_egg with storage rocketriders:main items
