...
execute if score $base_item_limit.tipped_arrow_slowness match_components matches 1.. run scoreboard players operation $item_limit var = $base_item_limit.tipped_arrow_slowness match_components
...
execute if score $item_batch_size.tipped_arrow_slowness match_components matches 1.. run scoreboard players operation $intended_batch_size var = $item_batch_size.tipped_arrow_slowness match_components
...
# limit reached
execute if score $give_item var matches 1 if score $final_batch_size var matches 0 if score $item_limit var matches 1 run title @s actionbar {color:"red",text:"Slowness Arrow already obtained"}
execute if score $give_item var matches 1 if score $final_batch_size var matches 0 if score $item_limit var matches 2.. run title @s actionbar {color:"red",text:"Maximum Slowness Arrows already obtained"}
...
execute if predicate game:game_rules/show_debug_logs/on run function custom:log {message:["(items:give_batch/tipped_arrow_slowness) Gave Slowness Arrow x",{score:{name:"$final_batch_size",objective:"var"}}," to ",{selector:"@s"}]}
