...
loot give @s loot {pools:[{rolls:1,entries:[{type:"minecraft:loot_table",value:"items:item/tipped_arrow_blindness"}],modifier:{type:"minecraft:set_count",count:{type:"minecraft:score",target:{type:"minecraft:fixed",name:"$existing_slots_remainder"},score:"var"}}}]}
