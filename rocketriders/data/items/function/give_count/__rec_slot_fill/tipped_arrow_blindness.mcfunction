...
loot give @s loot {pools:[{rolls:1,entries:[{type:"minecraft:loot_table",value:"items:item/tipped_arrow_blindness"}],functions:[{function:"minecraft:set_count",count:{type:"minecraft:score",target:{type:"minecraft:fixed",name:"$existing_slots_remainder"},score:"var"}}]}]}
scoreboard players operation $count var -= $existing_slots_remainder var
execute if score $count var matches 1.. run function items:give_count/__rec_slot_fill/tipped_arrow_blindness
