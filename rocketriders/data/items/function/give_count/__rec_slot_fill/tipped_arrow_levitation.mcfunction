execute store result score $existing_slots_remainder var if items entity @s custom:player *[custom_data~{id:"tipped_arrow_levitation"},!count=64]
...
loot give @s loot {pools:[{rolls:1,entries:[{type:"minecraft:loot_table",value:"items:item/tipped_arrow_levitation"}],modifier:{type:"minecraft:set_count",count:{type:"minecraft:score",target:{type:"minecraft:fixed",name:"$existing_slots_remainder"},score:"var"}}}]}
