...
function items:give_count/__rec_slot_fill/tipped_arrow_slowness
...
execute if score $offhand var matches 1 if score $count var matches ..64 run return run loot replace entity @s weapon.offhand loot {pools:[{rolls:1,entries:[{type:"minecraft:loot_table",value:"items:item/tipped_arrow_slowness"}],modifier:{type:"minecraft:set_count",count:{type:"minecraft:score",target:{type:"minecraft:fixed",name:"$count"},score:"var"}}}]}

execute if score $offhand var matches 1 run loot replace entity @s weapon.offhand loot {pools:[{rolls:1,entries:[{type:"minecraft:loot_table",value:"items:item/tipped_arrow_slowness"}],modifier:{type:"minecraft:set_count",count:64}}]}
