...
execute if predicate game:game_rules/show_debug_logs/on if entity @s[scores={HasHur=0},tag=!fullHotbar] run function custom:log {message:["(items:missile/lightning/givehur) Gave Hurricane to ",{selector:"@s"}]}
execute if predicate game:game_rules/show_debug_logs/on unless entity @s[scores={HasHur=0},tag=!fullHotbar] run function custom:log {message:["(items:missile/lightning/givehur) Failed to give Hurricane to ",{selector:"@s"}]}
