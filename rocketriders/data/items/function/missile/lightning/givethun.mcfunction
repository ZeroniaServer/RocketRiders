...
execute if predicate game:game_rules/show_debug_logs/on if entity @s[scores={HasBolt=0},tag=!fullHotbar] run function custom:log {message:["(items:missile/lightning/givethun) Gave Thunderbolt to ",{selector:"@s"}]}
execute if predicate game:game_rules/show_debug_logs/on unless entity @s[scores={HasBolt=0},tag=!fullHotbar] run function custom:log {message:["(items:missile/lightning/givethun) Failed to give Thunderbolt to ",{selector:"@s"}]}
