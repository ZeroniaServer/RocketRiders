...
execute if entity @e[x=0,type=armor_stand,tag=Selection,tag=doStacking] run scoreboard players operation @s HasBolt %= 64 CmdData
execute if entity @e[x=0,type=armor_stand,tag=Selection,tag=doStacking] if entity @s[scores={HasBolt=1..63},tag=fullHotbar] run tag @s remove fullHotbar
execute if entity @e[x=0,type=armor_stand,tag=Selection,tag=doStacking] run scoreboard players set @s HasBolt 0
...
#Game tracking
execute unless entity @s[tag=BackBolt] run tag @e[x=0,type=armor_stand,tag=Selection,tag=tetrisTime] add givenBolt
