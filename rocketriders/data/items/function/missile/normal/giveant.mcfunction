...
#Antidupe check
execute store result score @s HasAnt run clear @s #custom:missile[custom_data~{spawn_egg:{type:"missile",missile:"ant"}}] 0
execute if entity @e[x=0,type=armor_stand,tag=Selection,limit=1,tag=doStacking] run scoreboard players operation @s HasAnt %= $64 constant
execute if entity @e[x=0,type=armor_stand,tag=Selection,limit=1,tag=doStacking] if entity @s[scores={HasAnt=1..63},tag=fullHotbar] run tag @s remove fullHotbar
execute if entity @e[x=0,type=armor_stand,tag=Selection,limit=1,tag=doStacking] run scoreboard players set @s HasAnt 0
...
loot give @s[scores={HasAnt=0},tag=!fullHotbar] loot items:missile/normal/ant
