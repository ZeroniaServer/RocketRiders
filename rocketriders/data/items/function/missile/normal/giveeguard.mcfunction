...
execute if predicate game:game_rules/item_stacking/on run scoreboard players operation @s HasGuard %= $64 constant
execute if predicate game:game_rules/item_stacking/on if entity @s[scores={HasGuard=1..63},tag=fullHotbar] run tag @s remove fullHotbar
execute if predicate game:game_rules/item_stacking/on run scoreboard players set @s HasGuard 0
