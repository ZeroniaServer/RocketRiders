...
execute if predicate game:game_rules/show_debug_logs/on if entity @s[scores={HasNull=0},tag=!fullHotbar] run function custom:log {message:["(items:item/missile/givenull) Gave Chronullifier to ",{selector:"@s"}]}
execute if predicate game:game_rules/show_debug_logs/on unless entity @s[scores={HasNull=0},tag=!fullHotbar] run function custom:log {message:["(items:item/missile/givenull) Failed to give Chronullifier to ",{selector:"@s"}]}
