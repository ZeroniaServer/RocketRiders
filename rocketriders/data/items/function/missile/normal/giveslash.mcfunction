...
execute if predicate game:game_rules/show_debug_logs/on if entity @s[scores={HasSlash=0},tag=!fullHotbar] run function custom:log {message:["(items:item/missile/giveslash) Gave Slasher to ",{selector:"@s"}]}
execute if predicate game:game_rules/show_debug_logs/on unless entity @s[scores={HasSlash=0},tag=!fullHotbar] run function custom:log {message:["(items:item/missile/giveslash) Failed to give Slasher to ",{selector:"@s"}]}
