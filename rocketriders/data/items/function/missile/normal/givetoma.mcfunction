...
execute if predicate game:game_rules/show_debug_logs/on if entity @s[scores={HasToma=0},tag=!fullHotbar] run function custom:log {message:["(items:item/missile/givetoma) Gave TomaTwo to ",{selector:"@s"}]}
execute if predicate game:game_rules/show_debug_logs/on unless entity @s[scores={HasToma=0},tag=!fullHotbar] run function custom:log {message:["(items:item/missile/givetoma) Failed to give TomaTwo to ",{selector:"@s"}]}
