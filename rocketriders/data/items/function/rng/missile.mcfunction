...
execute if predicate game:game_rules/show_debug_logs/on run function custom:log {message:["(items:rng/missile) Selected: ",{selector:"@e[limit=1,x=0,type=marker,tag=rng2,tag=rngSelected]"}]}

execute as @e[x=0,type=marker,tag=rngSelected,tag=rngHeavy] run function items:rng/missile/heavy
execute as @e[x=0,type=marker,tag=rngSelected,tag=rngNormal] run function items:rng/missile/normal
execute as @e[x=0,type=marker,tag=rngSelected,tag=rngLightning] run function items:rng/missile/lightning
execute as @e[x=0,type=marker,tag=rngSelected,tag=rngSpecial] run function items:rng/missile/special
