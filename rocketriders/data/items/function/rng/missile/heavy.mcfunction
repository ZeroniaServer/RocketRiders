...
execute as @e[x=0,type=marker,tag=rngSelected,tag=rngAux] as @a[x=0,tag=getItem,predicate=custom:team/any_playing_team] run function items:give/missile/auxiliary
execute as @e[x=0,type=marker,tag=rngSelected,tag=rngRift] as @a[x=0,tag=getItem,predicate=custom:team/any_playing_team] run function items:give/missile/rifter
execute as @e[x=0,type=marker,tag=rngSelected,tag=rngJbuster] as @a[x=0,tag=getItem,predicate=custom:team/any_playing_team] run function items:give/missile/juggerbuster
execute as @e[x=0,type=marker,tag=rngSelected,tag=rngWar] as @a[x=0,tag=getItem,predicate=custom:team/any_playing_team] run function items:give/missile/warhead
