...
execute store success score $team var if entity @a[limit=1,tag=spawn_egg.placer,predicate=!custom:team/blue]

execute if score $team var matches 0 run scoreboard players add @a[limit=1,tag=spawn_egg.placer] BSurpriseSpawned 1
execute if score $team var matches 1 run scoreboard players add @a[limit=1,tag=spawn_egg.placer] YSurpriseSpawned 1
