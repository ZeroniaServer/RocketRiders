...
#Antidupe check
execute store result score @s[tag=!itemDeducted] HasSurprise run clear @s #custom:missile[custom_data~{spawn_egg:{type:"random_missile"}}] 0
...
execute if entity @s[predicate=custom:team/yellow,scores={HasSurprise=..2},tag=!fullHotbar] run function items:give/surprise_egg {count:1}
