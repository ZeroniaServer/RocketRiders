...
#Antidupe check
execute store result score @s[tag=!itemDeducted] HasSurprise run clear @s cod_spawn_egg 0
execute if entity @e[x=0,type=armor_stand,tag=Selection,limit=1,tag=doStacking] run scoreboard players operation @s HasSurprise %= 64 CmdData
...
loot give @s[team=Yellow,scores={HasSurprise=..2},tag=!fullHotbar] loot items:missile/surprise/all
