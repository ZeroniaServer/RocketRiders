...
execute if predicate game:game_rules/show_debug_logs/on if entity @s[scores={HasICBM=0},tag=!fullHotbar] run function custom:log {message:["(items:missile/util/giveicbm) Gave ICBM to ",{selector:"@s"}]}
execute if predicate game:game_rules/show_debug_logs/on unless entity @s[scores={HasICBM=0},tag=!fullHotbar] run function custom:log {message:["(items:missile/util/giveicbm) Failed to give ICBM to ",{selector:"@s"}]}
