...
execute if predicate game:game_rules/item_stacking/on run scoreboard players operation @s HasObshield %= $64 constant
execute if predicate game:game_rules/item_stacking/on if entity @s[scores={HasObshield=1..63},tag=fullHotbar] run tag @s remove fullHotbar
execute if predicate game:game_rules/item_stacking/on run scoreboard players set @s HasObshield 0
