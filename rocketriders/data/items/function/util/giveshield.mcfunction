...
execute if predicate game:game_rules/show_debug_logs/on if entity @s[scores={HasShield=0},tag=!fullHotbar] run function custom:log {message:["(items:missile/util/giveshield) Gave Shield to ",{selector:"@s"}]}
execute if predicate game:game_rules/show_debug_logs/on unless entity @s[scores={HasShield=0},tag=!fullHotbar] run function custom:log {message:["(items:missile/util/giveshield) Failed to give Shield to ",{selector:"@s"}]}
