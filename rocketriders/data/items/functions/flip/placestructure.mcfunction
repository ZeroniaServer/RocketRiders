...
execute if entity @e[predicate=custom:indimension,type=armor_stand,tag=Selection,tag=chaseEnabled] at @s positioned ~ ~2 ~ run function rr_chase:game/whitemissile

##Add 1 to appropriate missile count
execute if entity @s[tag=bluemissile] run scoreboard players add @e[predicate=custom:indimension,type=armor_stand,tag=Selection] bMissileCount 1
execute if entity @s[tag=yellowmissile] run scoreboard players add @e[predicate=custom:indimension,type=armor_stand,tag=Selection] yMissileCount 1