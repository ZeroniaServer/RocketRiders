##Full hotbar check
execute as @a[predicate=custom:indimension] unless entity @s[team=!Yellow,team=!Blue] if entity @e[type=armor_stand,tag=Selection,tag=doHotbarLimit] run function items:invcount
execute as @a[predicate=custom:indimension] unless entity @s[team=!Yellow,team=!Blue] if entity @e[type=armor_stand,tag=Selection,tag=!doHotbarLimit] run scoreboard players set @s invCount 0
