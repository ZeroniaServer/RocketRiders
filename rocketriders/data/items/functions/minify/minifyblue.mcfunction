...
scoreboard players operation $minifyblue bMissileCount = @e[predicate=custom:indimension,type=armor_stand,tag=Selection,limit=1] bMissileCount
...
execute if entity @s[tag=minify] run scoreboard players remove @e[predicate=custom:indimension,type=armor_stand,tag=Selection,limit=1] bMissileCount 1
tag @s remove minify
