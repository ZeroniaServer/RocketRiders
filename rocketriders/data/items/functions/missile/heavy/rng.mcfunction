...
execute unless entity @e[type=armor_stand,tag=Selection,tag=doStacking] as @e[type=marker,tag=rngSelected,tag=rngAux] as @e[type=item] if data entity @s {Item:{id:"minecraft:mooshroom_spawn_egg"}} run kill @s
execute unless entity @e[type=armor_stand,tag=Selection,tag=doStacking] as @e[type=marker,tag=rngSelected,tag=rngRift] as @e[type=item] if data entity @s {Item:{id:"minecraft:salmon_spawn_egg"}} run kill @s
execute unless entity @e[type=armor_stand,tag=Selection,tag=doStacking] as @e[type=marker,tag=rngSelected,tag=rngJbuster] as @e[type=item] if data entity @s {Item:{id:"minecraft:zombie_horse_spawn_egg"}} run kill @s
execute unless entity @e[type=armor_stand,tag=Selection,tag=doStacking] as @e[type=marker,tag=rngSelected,tag=rngWar] as @e[type=item] if data entity @s {Item:{id:"minecraft:silverfish_spawn_egg"}} run kill @s
