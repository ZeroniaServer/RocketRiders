...
execute if entity @e[x=0,type=armor_stand,tag=Selection,tag=doStacking] run scoreboard players operation @s HasBullet %= 64 CmdData
execute if entity @e[x=0,type=armor_stand,tag=Selection,tag=doStacking] if entity @s[scores={HasBullet=1..63},tag=fullHotbar] run tag @s remove fullHotbar
execute if entity @e[x=0,type=armor_stand,tag=Selection,tag=doStacking] run scoreboard players set @s HasBullet 0
...
#Game tracking
execute unless entity @s[tag=BackBull] run tag @e[x=0,type=armor_stand,tag=Selection,tag=tetrisTime] add givenBull
