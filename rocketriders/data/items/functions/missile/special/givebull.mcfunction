...
#Title/giving
title @s[scores={HasBullet=1..},tag=!fullHotbar,tag=!BackBull] actionbar {"text":"Bullet already obtained.","color":"light_purple"}
execute if entity @s[scores={HasBullet=1..},tag=!fullHotbar,tag=!BackBull] at @s run playsound minecraft:block.note_block.bass master @s ~ ~ ~ 1 1
title @s[scores={HasBullet=0},tag=!fullHotbar,tag=!BackBull] actionbar {"text":"Bullet obtained.","color":"light_purple"}
...
#Game tracking
execute unless entity @s[tag=BackBull] run tag @e[type=armor_stand,tag=Selection] add givenBull
execute unless entity @s[tag=BackBull] run tag @e[type=armor_stand,tag=Bot] add BotHasBull