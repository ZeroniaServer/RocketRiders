...
execute if entity @e[predicate=custom:indimension,type=armor_stand,tag=Selection,tag=doStacking,tag=!arrowLimit] run scoreboard players operation @s HasArrows %= 64 CmdData
execute if entity @e[predicate=custom:indimension,type=armor_stand,tag=Selection,tag=doStacking,tag=!arrowLimit] if entity @s[scores={HasArrows=1..63},tag=fullHotbar] run tag @s remove fullHotbar
execute if entity @e[predicate=custom:indimension,type=armor_stand,tag=Selection,tag=doStacking,tag=!arrowLimit] run scoreboard players set @s HasArrows 0
execute if entity @e[predicate=custom:indimension,type=armor_stand,tag=Selection,tag=!doStacking] if entity @s[scores={HasArrows=1..3},tag=fullHotbar] run tag @s remove fullHotbar
...
give @s[scores={HasArrows=0},tag=fullOffhand,tag=Infinity] arrow{display:{Name:'{"text":"Arrow","color":"light_purple","italic":true,"bold":true}'}} 1
...
item replace entity @s[scores={HasArrows=0},tag=!fullOffhand,tag=Infinity] weapon.offhand with arrow{display:{Name:'{"text":"Arrow","color":"light_purple","italic":true,"bold":true}'}} 1
...
#Game tracking
tag @e[predicate=custom:indimension,type=armor_stand,tag=Selection,tag=givenArrows,tag=tetrisTime] add givenArrowsTwice
tag @e[predicate=custom:indimension,type=armor_stand,tag=Selection,tag=tetrisTime] add givenArrows
scoreboard players set @e[predicate=custom:indimension,type=armor_stand,tag=Bot] botarrowitems 4