...
#Antidupe check
execute if entity @e[predicate=custom:indimension,type=armor_stand,tag=Selection,tag=!doStacking] store result score @s HasNova run clear @s crossbow{nova:1b} 0
execute if entity @e[predicate=custom:indimension,type=armor_stand,tag=Selection,tag=doStacking] run scoreboard players set @s HasNova 0
...
#Game tracking
tag @e[predicate=custom:indimension,type=armor_stand,tag=Selection,tag=tetrisTime] add givenNova