...
execute if entity @e[x=0,type=armor_stand,tag=Selection,limit=1,tag=EditedSettings] unless score $mcancel CmdData matches -1 if score $mcancelcount CmdData matches 2.. run tellraw @s [{"translate":"Cannot edit settings right now. Try again in %s seconds.","color":"red","with":[{"score":{"name":"$mcancelcount","objective":"CmdData"},"color":"#ff8585","bold":true}]}]
execute if entity @e[x=0,type=armor_stand,tag=Selection,limit=1,tag=EditedSettings] unless score $mcancel CmdData matches -1 if score $mcancelcount CmdData matches 1 run tellraw @s [{"translate":"Cannot edit settings right now. Try again in %s second.","color":"red","with":[{"text":"1","color":"#ff8585","bold":true}]}]
