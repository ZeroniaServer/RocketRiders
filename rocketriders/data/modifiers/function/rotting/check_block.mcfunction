...
# If the block can rot, isn't already rotting, and the cap is not full: start a rot at this block
execute unless block ~ ~ ~ #modifiers:can_rot unless block ~ ~ ~ minecraft:moving_piston{source:true} run return fail
execute align xyz unless entity @e[limit=1,dx=0,type=#entities:rot_brain_entity_type,predicate=entities:type/rot/brain] positioned ~0.5 ~0.5 ~0.5 summon marker run function entities:type/rot/init
