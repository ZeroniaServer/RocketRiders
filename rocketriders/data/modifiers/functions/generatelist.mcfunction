...
execute if entity @s[tag=Minute] run data modify storage rocketriders:modifiers list append value '{"text":"Minute Mix","color":"aqua","hoverEvent":{"action":"show_text","value":["",{"text":"Players receive a new set of 8 items every minute.","color":"white"}]}}'
execute if entity @s[tag=SurpriseEgg] run data modify storage rocketriders:modifiers list append value '{"text":"Surprise Eggs","color":"aqua","hoverEvent":{"action":"show_text","value":["",{"text":"Players can receive a spawn egg that spawns a random missile. These can stack up to 3.","color":"white"}]}}'
