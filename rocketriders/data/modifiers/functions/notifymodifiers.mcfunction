...
function modifiers:printlist

#Also notifies about Game Rules (because I'm lazy)
tellraw @a ["",{"text":"|","bold":true,"color":"dark_gray"}]
tellraw @a ["",{"text":"|","bold":true,"color":"dark_gray"},{"text":" Selected Game Rules","color":"gray"},{"text":" (hover for info)","color":"dark_gray","italic":true,"hoverEvent":{"action":"show_text","value":[{"text":"test"}]}}]

execute if entity @s[scores={servermode=0},tag=!Repeat] run tellraw @a [{"text":""}]
execute if entity @s[scores={servermode=1}] run tellraw @a ["",{"text":"|","bold":true,"color":"dark_gray"}]
