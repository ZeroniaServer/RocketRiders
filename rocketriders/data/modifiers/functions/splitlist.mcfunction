#Splits entire list of modifiers into individual lists based on line length
...
#Adds the next modifier in the list to the most recently created "split" list
execute if score $modifiersLeft CmdData matches 1.. run data modify storage rocketriders:modifiers split[-1] append from storage rocketriders:modifiers list[0]
execute if score $modifiersLeft CmdData matches 1.. run data remove storage rocketriders:modifiers list[0]
execute store result score $modifiersLeft CmdData run data get storage rocketriders:modifiers list

#Tracks line length and creates a new "split" list for the next line if it exceeds this value
...
execute if score $modifiersLeft CmdData matches 1.. if score $thisLineLength CmdData > $maxLineLength CmdData run data modify storage rocketriders:modifiers split append value []
