data modify storage rocketriders:main options.dialog set value {\
	...
	"action": {\
		...
		"action": {\
            ...
            "template": "trigger options set 10000$(default_spell)$(do_hotbar_auto_fill)$(particles)$(parkour_tips)$(tips)"\
		}\
	}\
}
...
execute unless predicate rr:has_parkour run data modify storage rocketriders:main options.dialog.action.action.template set value "trigger options set 10000$(default_spell)$(do_hotbar_auto_fill)$(particles)0$(tips)"

# Remove the default spell book option if crusade mode is not installed
...
execute unless entity @e[limit=1,x=0,type=armor_stand,tag=gamemodeAS,tag=rr_crusade] run execute if predicate rr:has_parkour run data modify storage rocketriders:main options.dialog.action.action.template set value "trigger options set 100000$(do_hotbar_auto_fill)$(particles)$(parkour_tips)$(tips)"
