...
execute as @s[tag=BlueBot,tag=!DoneSpawning,tag=BotAuxSpawn,tag=BotRider] at @s if block ~ ~-3 ~4 air unless entity @e[tag=RecentBotspawn,distance=..7] run tag @s add Speed17
...
execute as @s[tag=YellowBot,tag=!DoneSpawning,tag=BotAuxSpawn,tag=BotRider] at @s if block ~ ~-3 ~-4 air unless entity @e[tag=RecentBotspawn,distance=..7] run tag @s add Speed17
