...
execute as @a[x=0,tag=BlueNovaAttach,scores={novattach=15..},predicate=custom:is_on_ground] at @s run summon firework_rocket ~ ~ ~ {LifeTime:0,FireworksItem:{id:"firework_rocket",count:1,components:{"minecraft:fireworks":{flight_duration:3,explosions:[{shape:"star",colors:[15790320],fade_colors:[11743532,3887386,2437522,8073150,2651799,14188952,4312372,14602026,6719955,12801229,15435844],has_trail:true,has_twinkle:true}]}}}}

#Nova Rockets explode same team Canopies
tag @e[x=0,type=marker,tag=novatracker,tag=DontExplode] remove DontExplode
execute as @e[x=0,type=marker,tag=bluenovatracker,tag=!NovaLost,tag=!DontExplode] at @s if entity @e[predicate=entities:type/canopy,predicate=entities:origin_team/blue,distance=..7,limit=1] unless entity @e[type=firework_rocket,tag=BlueNova,limit=1,sort=nearest,distance=..5] run tag @s add CanopyIsGone
execute as @e[x=0,type=marker,tag=bluenovatracker,tag=CanopyIsGone,tag=!NovaLost,tag=!DontExplode] at @s if entity @e[predicate=entities:type/canopy,predicate=entities:origin_team/blue,distance=..7,limit=1] run tag @e[predicate=entities:type/canopy,predicate=entities:origin_team/blue,distance=..7,limit=1] add canopy.kill
...
execute as @e[x=0,type=marker,tag=CanopyIsGone,tag=yellownovatracker,tag=!NovaLost,tag=!DontExplode] at @s as @e[predicate=entities:type/canopy,predicate=entities:origin_team/blue,distance=..7,limit=1,sort=nearest] run tag @s add canopy.kill
...
execute if predicate game:modifiers/clutter_collector/on as @e[x=0,type=marker,tag=CanopyIsGone,tag=bluenovatracker,tag=!NovaLost,tag=!DontExplode] at @s as @e[predicate=entities:type/canopy,predicate=entities:origin_team/blue,distance=..7,limit=1,sort=nearest] at @s positioned ~ ~2 ~ run function custom:explosion {r:0,modifiers:{nbt:{Tags:["UtilKilled","NovaCanopy"],CustomName:"a Nova Rocket"}}}
execute unless predicate game:modifiers/clutter_collector/on if predicate game:modifiers/explosive/on as @e[x=0,type=marker,tag=CanopyIsGone,tag=bluenovatracker,tag=!NovaLost,tag=!DontExplode] at @s as @e[predicate=entities:type/canopy,predicate=entities:origin_team/blue,distance=..7,limit=1,sort=nearest] at @s positioned ~ ~2 ~ run function custom:explosion {r:7,modifiers:{nbt:{Tags:["UtilKilled","NovaCanopy"],CustomName:"a Nova Rocket"}}}
execute unless predicate game:modifiers/clutter_collector/on unless predicate game:modifiers/explosive/on as @e[x=0,type=marker,tag=CanopyIsGone,tag=bluenovatracker,tag=!NovaLost,tag=!DontExplode] at @s as @e[predicate=entities:type/canopy,predicate=entities:origin_team/blue,distance=..7,limit=1,sort=nearest] at @s positioned ~ ~2 ~ run function custom:explosion {r:5,modifiers:{nbt:{Tags:["UtilKilled","NovaCanopy"],CustomName:"a Nova Rocket"}}}
