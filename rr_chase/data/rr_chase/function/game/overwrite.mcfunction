...
execute if entity @s[tag=!Explosive,tag=!ClutterCollector] as @e[x=0,type=marker,tag=CanopyIsGone,tag=bluenovatracker,tag=!NovaLost,tag=!DontExplode] at @s as @e[predicate=entities:type/canopy,predicate=entities:origin_team/blue,distance=..7,limit=1,sort=nearest] at @s positioned ~ ~2 ~ run function custom:explosion {r:5,modifiers:{nbt:{Tags:["UtilKilled","NovaCanopy"],CustomName:"a Nova Rocket"}}}
