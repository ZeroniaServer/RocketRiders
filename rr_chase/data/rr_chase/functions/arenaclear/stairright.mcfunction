tp @e[predicate=custom:indimension,type=marker,tag=chaseStairs,limit=1] ~ ~ ~
...
execute positioned ~1 ~-1 ~ if entity @e[predicate=custom:indimension,type=marker,tag=chaseStairs,limit=1,predicate=custom:base_width] run function rr_chase:arenaclear/stairright