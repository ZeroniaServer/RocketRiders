...
execute if entity @s[scores={endtimer=1}] as @a[team=Blue,tag=Winner,limit=1] if entity @e[type=armor_stand,tag=chaseEnabled] run function rr_chase:chasegear/givegear
execute if entity @s[scores={endtimer=1}] as @a[team=Blue,tag=Winner,limit=1] if entity @e[type=armor_stand,tag=chaseEnabled] run item replace entity @s armor.chest with elytra{HideFlags:127,Enchantments:[{id:"binding_curse",lvl:1}],Unbreakable:1b,display:{Name:'[{"text":"Elytra","color":"blue","bold":true,"italic":false}]'}} 1
execute if entity @s[scores={endtimer=1}] run give @a[team=Blue,tag=Loser] bow{display:{Name:'{"translate":"Shooting Saber","color":"white","bold":true,"italic":false}',Lore:['{"translate":"A weapon used for both"}','{"translate":"ranged and melee attacks"}']},HideFlags:4,Unbreakable:1b,Enchantments:[{id:"flame",lvl:1}]}
