...
execute if entity @s[scores={endtimer=1}] as @a[predicate=custom:indimension,team=Blue,tag=Winner,limit=1] if entity @e[predicate=custom:indimension,type=armor_stand,tag=chaseEnabled] run function rr_chase:chasegear/givegear
execute if entity @s[scores={endtimer=1}] as @a[predicate=custom:indimension,team=Blue,tag=Winner,limit=1] if entity @e[predicate=custom:indimension,type=armor_stand,tag=chaseEnabled] run item replace entity @s armor.chest with elytra{HideFlags:127,Enchantments:[{id:"binding_curse",lvl:1}],Unbreakable:1b,display:{Name:'[{"text":"Elytra","color":"blue","bold":true,"italic":false}]'}} 1
