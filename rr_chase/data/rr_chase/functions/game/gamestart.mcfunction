...
execute if score $dust CmdData matches 1 if entity @s[tag=!BlueFull] as @e[type=marker,tag=bluejoinpad,tag=!CancelJoin] at @s run particle dust 3 3 3 1 ~ ~1 ~ 0.5 1 0.5 0.1 5 force @a[predicate=custom:indimension,predicate=!custom:belowroof]
execute if score $barriers CmdData matches 1 if entity @s[tag=BlueFull,tag=EditedSettings] as @e[type=marker,tag=bluejoinpad,tag=!CancelJoin] at @s run particle block_marker barrier ~ ~1 ~ 0 0 0 0 1 force @a[predicate=custom:indimension,predicate=!custom:belowroof]
