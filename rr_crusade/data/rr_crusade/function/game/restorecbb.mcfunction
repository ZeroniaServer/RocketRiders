...
execute unless score $CBB crusadehp matches 1.. as @a[x=0,team=Yellow] at @s run playsound minecraft:entity.experience_orb.pickup master @s ~ ~ ~ 1 0
execute unless score $CBB crusadehp matches 1.. as @a[x=0,team=Yellow] at @s run playsound minecraft:block.glass.break master @s ~ ~ ~ 1 2
execute unless score $CBB crusadehp matches 1.. as @a[x=0,team=Blue] at @s run playsound minecraft:entity.wither.death master @s ~ ~ ~ 1 1
execute unless score $CBB crusadehp matches 1.. as @a[x=0,team=Blue] at @s run playsound minecraft:block.glass.break master @s ~ ~ ~ 1 0

execute if score $CBB crusadehp matches 1.. unless score $nodeathmessages CmdData matches 1 run gamerule showDeathMessages false
execute if score $CBB crusadehp matches 1.. unless score $nodeathmessages CmdData matches 1 positioned -14 49 -37 as @e[type=player,gamemode=!creative,gamemode=!spectator,team=!Spectator,team=!Developer,team=!Lobby,distance=..5] run tellraw @a[x=0] ["",{"selector":"@s"},{"text":" got too close to an End Crystal","color":"white"}]
