...
execute unless predicate game:game_started unless predicate game:game_ended if score @e[limit=1,x=0,type=armor_stand,tag=Selection] count matches ..599 unless entity @s[scores={crusadekit=2}] run loot replace entity @s weapon.offhand loot {pools:[{rolls:1,entries:[{type:"loot_table",value:"items:misc/arrow"}],functions:[{function:"set_count",count:20}]}]}
execute if predicate game:game_started unless predicate game:game_ended run function rr_crusade:items/util/givearrows
