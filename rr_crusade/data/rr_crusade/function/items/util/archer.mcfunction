...
execute as @e[x=0,type=marker,tag=rngSelected,tag=rngArrows] as @a[x=0,predicate=rr_crusade:kit/archer,tag=getItem,predicate=custom:team/any_playing_team] run function items:item/givearrows
execute as @e[x=0,type=marker,tag=rngSelected,tag=rngCanopy] as @a[x=0,predicate=rr_crusade:kit/archer,tag=getItem,predicate=custom:team/any_playing_team] run function items:item/givecanopy
execute as @e[x=0,type=marker,tag=rngSelected,tag=rngSplash] as @a[x=0,predicate=rr_crusade:kit/archer,tag=getItem,predicate=custom:team/any_playing_team] run function items:item/givesplash
