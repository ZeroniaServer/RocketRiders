...
execute as @e[x=0,type=marker,tag=rngSelected,tag=rngObshield] as @a[x=0,scores={crusadekit=1},tag=getItem,predicate=custom:team/any_playing_team] run function items:util/giveobshield
execute as @e[x=0,type=marker,tag=rngSelected,tag=rngShield] as @a[x=0,scores={crusadekit=1},tag=getItem,predicate=custom:team/any_playing_team] run function items:util/giveshield
execute as @e[x=0,type=marker,tag=rngSelected,tag=rngICBM] as @a[x=0,scores={crusadekit=1},tag=getItem,predicate=custom:team/any_playing_team] run function items:util/giveicbm
