...
execute as @a[predicate=custom:indimension,team=!Spectator,scores={crusadekit=2}] at @s as @e[type=arrow,scores={arrowtime=10..},nbt={inGround:1b},tag=!infinity,limit=1,distance=..2,sort=nearest] unless data entity @s CustomPotionEffects run tag @s add pickup
...
#Give arrow
execute as @a[predicate=custom:indimension,team=!Spectator,tag=!fullOffhand,scores={crusadekit=2}] at @s as @e[type=arrow,tag=pickup,tag=!given,nbt={inGround:1b},tag=!infinity,limit=1,distance=..2,sort=nearest] unless data entity @s CustomPotionEffects at @s run function everytick:arrow_replace
