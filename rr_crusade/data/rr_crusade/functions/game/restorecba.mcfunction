...
execute if score $CBA crusadehp matches 1.. positioned 38 49 -37 as @e[type=player,team=!Spectator,team=!Developer,team=!Lobby,distance=..5] run tellraw @a ["",{"selector":"@s"},{"text":" went too close to an End Crystal","color":"white"}]
...
execute unless score $CBA crusadehp matches 1.. run tellraw @a [""]