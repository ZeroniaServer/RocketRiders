...
execute if score $BlueShield crusadehp matches -1000..0 as @e[predicate=custom:indimension,type=egg,tag=BlueICBM,tag=teleportPair] at @s if entity @s[x=1,y=42,z=-73,dx=22,dy=27,dz=12] run kill @e[predicate=custom:indimension,type=marker,tag=ICBMtracker,tag=teleportPair]
execute if score $YellowShield crusadehp matches -1000..0 as @e[predicate=custom:indimension,type=egg,tag=YellowICBM,tag=teleportPair] at @s if entity @s[x=1,y=42,z=61,dx=22,dy=27,dz=12] run kill @e[predicate=custom:indimension,type=marker,tag=ICBMtracker,tag=teleportPair]
