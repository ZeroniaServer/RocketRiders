...
execute if entity @e[type=armor_stand,tag=Selection,scores={servermode=0},tag=!SMCustom] as @a[predicate=custom:indimension,team=Yellow,scores={HolyLand=5..}] run advancement grant @s only achievements:rr_challenges/holyland
