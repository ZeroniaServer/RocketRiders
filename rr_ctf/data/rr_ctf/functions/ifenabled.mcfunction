...
execute if entity @e[tag=PlacerClear,tag=Cleared,type=area_effect_cloud] if entity @s[tag=!GameStarted] run tellraw @a ["",{"text":"|","bold":true,"color":"dark_gray"},{"text":" Selected Gamemode: ","color":"gray"},{"text":"Capture the Flag ","color":"light_purple"},{"text":"(hover for info)","italic":true,"color":"dark_gray","hoverEvent":{"action":"show_text","value":["",{"text":"In Capture the Flag mode, there are no portals, bases are made out of concrete, and players (equipped with Piercing Pickaxes) must ride to the enemy base and capture both of their flags. To capture a flag, climb onto the enemy base and break the purple glass block on either flagpole to lower the flag, at which point it can be stolen. Once someone steals a flag, their missiles spawn in the opposite direction so they can go back to their own base and successfully capture the flag. The first team to capture both enemy flags wins.","color":"white"}]}}]
