...
execute as @s[tag=!gaveFirstItem] run scoreboard players set @e[type=marker,tag=CTFRNG,limit=1] RNGscore 3

#Give canopy (25% chance) - cannot have a streak greater than 2 canopies in a row
execute as @s[tag=gaveFirstItem,tag=!canopyStreaked,scores={canopyStreak=..1}] if entity @e[type=marker,tag=CTFRNG,limit=1,scores={RNGscore=0}] as @a unless entity @s[team=!Yellow,team=!Blue] run function items:util/givecanopy
execute as @s[tag=gaveFirstItem,tag=!canopyStreaked,scores={canopyStreak=..1}] if entity @e[type=marker,tag=CTFRNG,limit=1,scores={RNGscore=0}] run scoreboard players add @s canopyStreak 1
execute as @s[tag=canopyStreaked] if entity @e[type=marker,tag=CTFRNG,limit=1,scores={RNGscore=0}] run function items:rng
execute as @s[tag=canopyStreaked] if entity @e[type=marker,tag=CTFRNG,limit=1,scores={RNGscore=0}] run function items:tetris
execute as @s[tag=canopyStreaked,scores={canopyStreak=1..}] if entity @e[type=marker,tag=CTFRNG,limit=1,scores={RNGscore=0}] run scoreboard players remove @s canopyStreak 1
execute as @s[tag=gaveFirstItem,tag=!canopyStreaked,scores={canopyStreak=2..}] if entity @e[type=marker,tag=CTFRNG,limit=1,scores={RNGscore=0}] run tag @s add canopyStreaked
...
execute as @s[scores={canopyStreak=1..}] if entity @e[type=marker,tag=CTFRNG,limit=1,scores={RNGscore=1..3}] run scoreboard players remove @s canopyStreak 1
