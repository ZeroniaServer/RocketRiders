...
execute unless predicate game:game_started if entity @e[x=0,type=armor_stand,tag=Selection,limit=1,tag=!noYZELO] as @a[x=0,team=!Blue,team=!Yellow,tag=InRanked] run function rr_duel:forfeit/giveloss