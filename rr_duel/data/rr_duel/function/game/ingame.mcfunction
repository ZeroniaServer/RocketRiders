...
execute if entity @s[tag=TimeOut] unless entity @s[scores={servermode=0}] run tellraw @a[x=0] ["",{"text":"[FORFEIT] ","bold":true,"color":"dark_red"},{"text":"Someone left the 1v1 Duel match, so it has ended.","color":"red"}]
execute if entity @s[tag=TimeOut] unless entity @s[scores={servermode=0}] run scoreboard players set @s ForfeitTimeout 1200

#adds original player back (non-servermode)
execute if entity @s[scores={servermode=0}] as @a[x=0,tag=InRanked,tag=WasInBlue,team=Lobby,tag=!Forfeiter,limit=1] unless entity @a[x=0,team=Blue] run tellraw @s {"text":"You were added back to Blue to finish the 1v1 Duel match.","color":"red","italic":true}
execute if entity @s[scores={servermode=0}] as @a[x=0,tag=InRanked,tag=WasInBlue,team=Lobby,tag=!Forfeiter,limit=1] unless entity @a[x=0,team=Blue] run function game:joinblue
execute if entity @s[scores={servermode=0}] as @a[x=0,tag=InRanked,tag=WasInYellow,team=Lobby,tag=!Forfeiter,limit=1] unless entity @a[x=0,team=Yellow] run tellraw @s {"text":"You were added back to Yellow to finish the 1v1 Duel match.","color":"red","italic":true}
execute if entity @s[scores={servermode=0}] as @a[x=0,tag=InRanked,tag=WasInYellow,team=Lobby,tag=!Forfeiter,limit=1] unless entity @a[x=0,team=Yellow] run function game:joinyellow
...
execute if entity @s[tag=TimeOut] run clear @a[x=0,team=Blue] crossbow[custom_data~{nova:1b}]
