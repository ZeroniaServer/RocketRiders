...
execute if score @s servermode matches 0 if entity @s[tag=!noYZELO] run tellraw @a[x=0,scores={dropBow=1..}] {"text":"Note: you will lose XP!","color":"dark_red","italic":true}
execute if score @s servermode matches 0 run scoreboard players set @a[x=0,scores={dropBow=1..}] dropBow 0
execute if score @s servermode matches 0 as @a[x=0,team=Blue,scores={LeaveMidgame=1}] run tellraw @a[x=0] [{"selector":"@s"},{"text":" forfeited the match!","color":"red"}]
execute if score @s servermode matches 0 as @a[x=0,team=Blue,scores={LeaveMidgame=1}] run tag @s add Forfeiter
execute if score @s servermode matches 0 as @a[x=0,team=Yellow,scores={LeaveMidgame=1}] run tellraw @a[x=0] [{"selector":"@s"},{"text":" forfeited the match!","color":"red"}]
execute if score @s servermode matches 0 as @a[x=0,team=Yellow,scores={LeaveMidgame=1}] run tag @s add Forfeiter
