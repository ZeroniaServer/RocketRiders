...
execute unless predicate game:phase/match/over unless entity @s[tag=NOFORFEITS] unless score Blue: RoundsWon matches 2.. unless score Yellow: RoundsWon matches 2.. if entity @a[limit=1,x=0,predicate=custom:team/yellow] unless entity @a[limit=1,x=0,predicate=custom:team/blue] run scoreboard players set $1v1_duel_time_out_period global 1
execute unless predicate game:phase/match/over unless entity @s[tag=NOFORFEITS] unless score Blue: RoundsWon matches 2.. unless score Yellow: RoundsWon matches 2.. unless entity @a[limit=1,x=0,predicate=custom:team/yellow] if entity @a[limit=1,x=0,predicate=custom:team/blue] run scoreboard players set $1v1_duel_time_out_period global 1
execute unless predicate game:phase/match/over if predicate rr_duel:time_out_period if predicate rr:is_cubekrowd run tellraw @a[x=0] ["",{bold:true,color:"dark_red",text:"[FORFEIT] "},{color:"red",text:"Someone left the 1v1 Duel match, so it has ended."}]
execute unless predicate game:phase/match/over if predicate rr_duel:time_out_period if predicate rr:is_cubekrowd run scoreboard players set @s ForfeitTimeout 1200

#adds original player back (non-servermode)
execute unless predicate game:phase/match/over unless predicate rr:is_cubekrowd as @e[limit=2,x=-0.5,y=-0.5,z=-0.5,dx=0,dy=0,dz=0,type=area_effect_cloud,tag=1v1_duel_player_tracker] run function rr_duel:game/player_tracking/check

#timeout
execute unless predicate game:phase/match/over if predicate rr_duel:time_out_period run function rr_duel:game/while_time_out
...
execute if predicate game:phase/match/over run bossbar remove rr_duel:forfeit_timer
execute if predicate game:phase/match/over run scoreboard players reset @s ForfeitTimeout
...
##double forfeit
execute unless predicate game:phase/match/over unless score Blue: RoundsWon matches 2.. unless score Yellow: RoundsWon matches 2.. unless entity @a[limit=1,x=0,predicate=custom:team/any_playing_team] run function game:forcestop
