...
scoreboard players operation @a[x=0,team=Blue,limit=1] XP += $RankResult XP
...
execute as @a[x=0,team=Blue,limit=1] run tellraw @a[x=0] ["",{"selector":"@s","color":"green"},{"text":" gained ","color":"green"},{"score":{"name":"$NetBlue","objective":"XP"},"bold":true,"color":"dark_green"},{"text":", making their XP a total of: ","color":"green"},{"score":{"name":"@s","objective":"XP"},"bold":true,"color":"light_purple"}]
...
execute if entity @a[x=0,team=Yellow,scores={XP=..-1},limit=1] run scoreboard players operation $NetYellow XP += @a[x=0,team=Yellow,limit=1] XP
scoreboard players set @a[x=0,team=Yellow,scores={XP=..-1},limit=1] XP 0

#Announce new Yellow XP
execute as @a[x=0,team=Yellow,limit=1] run tellraw @a[x=0] ["",{"selector":"@s","color":"red"},{"text":" lost ","color":"red"},{"score":{"name":"$NetYellow","objective":"XP"},"bold":true,"color":"dark_red"},{"text":", making their XP a total of: ","color":"red"},{"score":{"name":"@s","objective":"XP"},"bold":true,"color":"light_purple"}]
