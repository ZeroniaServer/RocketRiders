...
execute if entity @a[x=0,predicate=custom:team/yellow,limit=1] run scoreboard players operation $CurrentBlue XP *= @a[x=0,predicate=custom:team/yellow,limit=1] RankScore
...
scoreboard players operation @a[x=0,predicate=custom:team/yellow,limit=1] XP += $RankResult XP
scoreboard players operation $NetYellow XP += $RankResult XP

#Adjust Yellow XP to below 1301
execute if entity @a[x=0,predicate=custom:team/yellow,scores={XP=1301..},limit=1] run scoreboard players operation $DiffYellow XP = @a[x=0,predicate=custom:team/yellow,limit=1,scores={XP=1301..}] XP
execute if entity @a[x=0,predicate=custom:team/yellow,scores={XP=1301..},limit=1] run scoreboard players operation $DiffYellow XP -= $1300 constant
execute if entity @a[x=0,predicate=custom:team/yellow,scores={XP=1301..},limit=1] run scoreboard players operation $NetYellow XP -= $DiffYellow XP
scoreboard players set @a[x=0,predicate=custom:team/yellow,scores={XP=1301..},limit=1] XP 1300

#Announce new Yellow XP
execute as @a[x=0,predicate=custom:team/yellow,limit=1] run tellraw @a[x=0] ["",{"selector":"@s","color":"green"},{"text":" gained ","color":"green"},{"score":{"name":"$NetYellow","objective":"XP"},"bold":true,"color":"dark_green"},{"text":", making their XP a total of: ","color":"green"},{"score":{"name":"@s","objective":"XP"},"bold":true,"color":"light_purple"}]
...
scoreboard players operation @a[x=0,predicate=custom:team/blue,limit=1] XP -= $LossXP XP
...
scoreboard players operation @a[x=0,predicate=custom:team/blue,limit=1] XP += $Buffer XP
scoreboard players operation $NetBlue XP -= $Buffer XP

#Adjust Blue XP to above 0
execute if entity @a[x=0,predicate=custom:team/blue,scores={XP=..-1},limit=1] run scoreboard players operation $NetBlue XP += @a[x=0,predicate=custom:team/blue,limit=1] XP
scoreboard players set @a[x=0,predicate=custom:team/blue,scores={XP=..0},limit=1] XP 0

#Announce new Blue XP
execute as @a[x=0,predicate=custom:team/blue,limit=1] run tellraw @a[x=0] ["",{"selector":"@s","color":"red"},{"text":" lost ","color":"red"},{"score":{"name":"$NetBlue","objective":"XP"},"bold":true,"color":"dark_red"},{"text":", making their XP a total of: ","color":"red"},{"score":{"name":"@s","objective":"XP"},"bold":true,"color":"light_purple"}]
