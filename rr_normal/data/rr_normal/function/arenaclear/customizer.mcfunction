...
data modify block -69 192 74 front_text.messages[1] set value '{"text":"Normal","color":"light_purple","clickEvent":{"action":"run_command","value":"execute if entity @e[x=0,type=armor_stand,tag=Selection,limit=1,scores={refreshsigns=..0}] run playsound ui.button.click master @a[x=0] ~ ~ ~ 1 1"}}'
