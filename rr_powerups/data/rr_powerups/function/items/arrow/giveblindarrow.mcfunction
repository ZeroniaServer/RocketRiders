...
execute if predicate game:game_rules/show_debug_logs/on if entity @s[scores={HasTippedArrows=..3},tag=!fullHotbar] run function custom:log {message:["(rr_powerups:items/arrow/giveblindarrow) Gave Blindness Arrow(s) to ",{selector:"@s"}]}
execute if predicate game:game_rules/show_debug_logs/on unless entity @s[scores={HasTippedArrows=..3},tag=!fullHotbar] run function custom:log {message:["(rr_powerups:items/arrow/giveblindarrow) Failed to give Blindness Arrow(s) to ",{selector:"@s"}]}
title @s[scores={HasTippedArrows=..3},tag=!fullHotbar] actionbar {"text":"Blindness Arrows obtained.","color":"light_purple"}
execute if score @s HasTippedArrows matches 0 if entity @s[tag=!fullHotbar] run function items:give/tipped_arrow_blindness {count:4}
execute if score @s HasTippedArrows matches 1 if entity @s[tag=!fullHotbar] run function items:give/tipped_arrow_blindness {count:3}
execute if score @s HasTippedArrows matches 2 if entity @s[tag=!fullHotbar] run function items:give/tipped_arrow_blindness {count:2}
execute if score @s HasTippedArrows matches 3 if entity @s[tag=!fullHotbar] run function items:give/tipped_arrow_blindness {count:1}
