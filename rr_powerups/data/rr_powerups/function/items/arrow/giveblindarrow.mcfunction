...
execute if items entity @s[tag=!fullOffhand] container.* tipped_arrow[count~{max:63},potion_contents={custom_effects:[{show_icon:1b,duration:200,id:"minecraft:blindness"}]}] run tag @s add fullOffhand

give @s[scores={HasTippedArrows=0},tag=!fullHotbar,tag=fullOffhand] minecraft:tipped_arrow[custom_name='{"text":"Blindness Arrow","color":"dark_purple","italic":true,"bold":true}',potion_contents={custom_effects:[{id:"minecraft:blindness",amplifier:0b,duration:200,show_particles:1b}]},hide_additional_tooltip={},custom_data={Droppable:1b}] 4
