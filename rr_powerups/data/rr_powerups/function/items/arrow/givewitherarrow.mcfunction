...
execute if predicate game:game_rules/show_debug_logs/on if entity @s[scores={HasTippedArrows=..3},tag=!fullHotbar] run function custom:log {message:["(rr_powerups:items/arrow/givewitherarrow) Gave Wither Arrow(s) to ",{selector:"@s"}]}
execute if predicate game:game_rules/show_debug_logs/on unless entity @s[scores={HasTippedArrows=..3},tag=!fullHotbar] run function custom:log {message:["(rr_powerups:items/arrow/givewitherarrow) Failed to give Wither Arrow(s) to ",{selector:"@s"}]}
