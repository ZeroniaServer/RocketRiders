...
execute if entity @e[x=0,type=armor_stand,tag=Selection,limit=1,tag=doStacking] run scoreboard players operation @s HasLavaSplash %= 64 CmdData
...
give @s[team=Yellow,scores={HasLavaSplash=0},tag=!fullHotbar] minecraft:lingering_potion[max_stack_size=64,custom_data={lavasplash:1b},custom_name='[{"text":"[Utility] ","color":"dark_purple","bold":true},{"text":"Lava Splash","color":"gold","bold":true}]',lore=['{"text":"Spawns a brief lava source where it","color":"gray"}','{"text":"lands. Used to burn enemies or break","color":"gray"}','{"text":"your fall in a dumb way.","color":"gray"}'],enchantments={levels:{"minecraft:unbreaking":1},show_in_tooltip:0b},potion_contents={potion:"minecraft:awkward",custom_effects:[{id:"minecraft:saturation",amplifier:0,duration:1,show_particles:0b}],custom_color:16747545},hide_additional_tooltip={}] 1
