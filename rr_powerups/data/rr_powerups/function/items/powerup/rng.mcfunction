...
execute unless entity @e[x=0,type=armor_stand,tag=Selection,limit=1,tag=doStacking] as @e[x=0,type=marker,tag=rngSelected,tag=rngBeeShield] as @e[x=0,type=item] if items entity @s contents snowball[custom_data~{BeeShield:1b}] run function rr_powerups:items/deduct
