...
execute as @s[scores={endtimer=1..}] run scoreboard players set @e[type=marker,tag=captureMiddle] capturePoint 0
execute as @s[scores={endtimer=1..}] run kill @e[type=bee]
execute as @s[scores={endtimer=569..}] run kill @e[type=armor_stand,tag=BeeShieldDisplay]
execute as @s[scores={endtimer=569..}] run kill @e[type=marker,tag=animBshield]
