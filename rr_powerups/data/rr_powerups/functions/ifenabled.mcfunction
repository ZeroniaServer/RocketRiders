#management
tag @e[predicate=custom:indimension,type=armor_stand,tag=rr_powerups,limit=1] add enabled
execute if entity @s[tag=SignsRefreshed] run function rr_powerups:arenaclear/customizer
execute unless score @s SetGamemode = @e[predicate=custom:indimension,type=armor_stand,tag=rr_powerups,limit=1] gamemodeID run function rr_powerups:disable
...
execute if entity @e[predicate=custom:indimension,type=arrow,tag=!GameEnd] run function rr_powerups:everytick/tipped_arrow_pickup

#reset
execute if entity @e[predicate=custom:indimension,type=marker,tag=PlacerClear,tag=Cleared] run function rr_powerups:arenaclear/baseplacement
...
execute if entity @e[predicate=custom:indimension,type=marker,tag=PlacerClear,tag=Cleared] if entity @s[tag=!GameStarted] run tag @s add powerupsLast
...
execute if entity @s[tag=EditedSettings] run tellraw @a[predicate=custom:indimension,tag=informMe] ["",{"text":"|","bold":true,"color":"dark_gray"},{"text":" Selected Gamemode: ","color":"gray"},{"text":"Powerups","color":"light_purple","hoverEvent":{"action":"show_text","contents":["",{"text":"Objective:","color":"gold"},{"text":" Destroy enemy portals\n","color":"yellow"},{"text":"Specifics:\n","color":"dark_aqua"},{"text":"- Stand on Powerup Platform for 3 seconds to capture it\n"},{"text":"- The team that controls the Platform gets Powerup Items\n"},{"text":"- Platform becomes contested if both teams are on it\n"},{"text":"- Crystal Crafters appear when enemies control the Powerup Platform\n"},{"text":"- Stand on Crystal Crafter to launch Crystals at Platform\n"},{"text":"- The Platform slowly breaks over time\n"},{"text":"- Crystals significantly speed up Platform breaking process\n"},{"text":"Powerup Items:\n","color":"dark_purple"},{"text":"Special Missiles, Tipped Arrows, Stinging Shield, Lava Splash, Trident, Elytra, Totem of Undying, Infinity Saber, Slap Fish"}]}},{"text":" (hover name for info)","italic":true,"color":"dark_gray"}]
...
execute unless entity @e[predicate=custom:indimension,type=marker,tag=captureMiddle,tag=BrokenPlatform] run bossbar set rr_powerups:capture_progress max 100