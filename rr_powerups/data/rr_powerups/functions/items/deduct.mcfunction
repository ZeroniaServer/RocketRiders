...
execute if entity @e[type=marker,scores={capturePoint=1}] run tag @a[team=Blue,predicate=custom:matches_uuid] add correctTeam
execute if entity @e[type=marker,scores={capturePoint=2}] run tag @a[team=Yellow,predicate=custom:matches_uuid] add correctTeam
...
tag @a[tag=correctTeam] add itemDeducted
tag @a[tag=correctTeam,tag=fullHotbar] add wasFullHotbar
