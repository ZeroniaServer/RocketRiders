...
execute if entity @e[predicate=custom:indimension,type=marker,scores={capturePoint=1}] run tag @a[predicate=custom:indimension,team=Blue,predicate=custom:matches_uuid] add correctTeam
execute if entity @e[predicate=custom:indimension,type=marker,scores={capturePoint=2}] run tag @a[predicate=custom:indimension,team=Yellow,predicate=custom:matches_uuid] add correctTeam
