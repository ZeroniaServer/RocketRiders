...
execute if entity @e[type=marker,scores={capturePoint=1}] as @a[team=Blue,predicate=custom:matches_uuid,tag=!itemDeducted] run tag @s add itemDeducted
execute if entity @e[type=marker,scores={capturePoint=2}] as @a[team=Yellow,predicate=custom:matches_uuid,tag=!itemDeducted] run tag @s add itemDeducted
