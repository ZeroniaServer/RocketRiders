#Antidupe check
execute if entity @e[type=armor_stand,tag=Selection,tag=!doStacking] store result score @s HasLavaSplash run clear @s lingering_potion{Potion:"minecraft:awkward"} 0
execute if entity @e[type=armor_stand,tag=Selection,tag=doStacking] run scoreboard players set @s HasLavaSplash 0

#Title/giving
title @s[scores={HasLavaSplash=1..},tag=!fullHotbar] actionbar {"text":"Lava Splash already obtained.","color":"light_purple"}
execute if entity @s[scores={HasLavaSplash=1..},tag=!fullHotbar] at @s run playsound minecraft:block.note_block.bass master @s ~ ~ ~ 1 1
...
tag @e[type=armor_stand,tag=Selection] add givenLavaSplash