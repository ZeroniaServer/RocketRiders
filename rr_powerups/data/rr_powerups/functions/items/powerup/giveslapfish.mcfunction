...
execute if entity @e[type=armor_stand,tag=Selection,tag=doStacking] as @s[scores={HasSlapFish=1..63},tag=fullHotbar] run tag @s remove fullHotbar
...
execute as @s[scores={HasSlapFish=1..},tag=!fullHotbar] at @s run playsound minecraft:block.note_block.bass master @s ~ ~ ~ 1 1
