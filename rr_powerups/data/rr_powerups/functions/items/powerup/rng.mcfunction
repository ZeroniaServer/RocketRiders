...
execute unless entity @e[type=armor_stand,tag=Selection,tag=doStacking] as @e[type=marker,tag=rngSelected,tag=rngLavaSplash] as @e[type=item] if data entity @s {Item:{id:"minecraft:lingering_potion",tag:{Potion:"minecraft:awkward",CustomPotionColor:16747545}}} run function items:deduct
execute unless entity @e[type=armor_stand,tag=Selection,tag=doStacking] as @e[type=marker,tag=rngSelected,tag=rngSlapFish] as @e[type=item] if data entity @s {Item:{id:"minecraft:cooked_salmon"}} run function items:deduct
execute unless entity @e[type=armor_stand,tag=Selection,tag=doStacking] as @e[type=marker,tag=rngSelected,tag=rngTotem] as @e[type=item] if data entity @s {Item:{id:"minecraft:totem_of_undying"}} run kill @s
execute unless entity @e[type=armor_stand,tag=Selection,tag=doStacking] as @e[type=marker,tag=rngSelected,tag=rngTrident] as @e[type=item] if data entity @s {Item:{id:"minecraft:trident"}} run kill @s
execute unless entity @e[type=armor_stand,tag=Selection,tag=doStacking] as @e[type=marker,tag=rngSelected,tag=rngBeeShield] as @e[type=item] if data entity @s {Item:{id:"minecraft:honey_bottle"}} run function items:deduct
