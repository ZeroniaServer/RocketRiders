...
execute as @e[tag=SelectedTip,tag=Tip8,type=marker] as @a unless entity @s[scores={GamesPlayed=10..}] run tellraw @s ["",{"text":"<","color":"dark_gray"},{"text":"TIP!","bold":true,"color":"green"},{"text":"> ","color":"dark_gray"},{"text":"If the enemy team has control of the Powerup Platform, your team gets a Crystal Platform. Stand on top to send an End Crystal to the Platform!","color":"gray"}]
