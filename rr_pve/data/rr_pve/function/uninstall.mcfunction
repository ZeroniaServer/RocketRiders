...
execute unless entity @e[x=0,type=armor_stand,tag=rr_pve,limit=1] run tellraw @s {"text":"Click here to disable the PVE Mode datapack (recommended).","color":"red","underlined":true,"click_event":{"action":"run_command","command":"/datapack disable 'file/rr_pve'"}}
execute unless entity @e[x=0,type=armor_stand,tag=rr_pve,limit=1] run tellraw @s {"text":"Click here to disable the Bots datapack (recommended).","color":"red","underlined":true,"click_event":{"action":"run_command","command":"/datapack disable 'file/rr_bots'"}}
