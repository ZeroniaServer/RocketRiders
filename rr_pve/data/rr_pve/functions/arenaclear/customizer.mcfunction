...
data modify block -69 192 74 front_text.messages[1] set value '{"text":"PVE","color":"light_purple","clickEvent":{"action":"run_command","value":"execute if entity @e[predicate=custom:indimension,type=armor_stand,tag=Selection,scores={refreshsigns=..0}] run playsound ui.button.click master @a[predicate=custom:indimension] ~ ~ ~ 1 1"}}'
