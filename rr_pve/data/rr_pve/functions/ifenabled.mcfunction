#management
tag @e[predicate=custom:indimension,type=armor_stand,tag=rr_pve,limit=1] add enabled
execute if entity @s[tag=SignsRefreshed] run function rr_pve:arenaclear/customizer
execute unless score @s SetGamemode = @e[predicate=custom:indimension,type=armor_stand,tag=rr_pve,limit=1] gamemodeID run function rr_pve:disable
...
#reset
execute if entity @e[predicate=custom:indimension,type=marker,tag=PlacerClear,tag=Cleared] run function rr_pve:arenaclear/baseplacement
...
execute if entity @e[predicate=custom:indimension,type=marker,tag=PlacerClear,tag=Cleared] if entity @s[tag=!GameStarted] run tag @s add pveLast
...
execute if entity @s[tag=EditedSettings] run tellraw @a[predicate=custom:indimension,tag=informMe] ["",{"text":"|","bold":true,"color":"dark_gray"},{"text":" Selected Gamemode: ","color":"gray"},{"text":"PVE","color":"light_purple","hoverEvent":{"action":"show_text","contents":["",{"text":"Objective:","color":"gold"},{"text":" Destroy enemy portals\n","color":"yellow"},{"text":"- Ride missiles to enemy base to destroy portals\n"},{"text":"- Protect your own team's portals\n"},{"text":"Specifics:\n","color":"dark_aqua"},{"text":"- Yellow team consists of 12 artificially intelligent Bots\n"},{"text":"- Bots can spawn and ride missiles to your base\n"},{"text":"- Bots can fire arrows at you if you're in sight\n"}]}},{"text":" (hover name for info)","italic":true,"color":"dark_gray"}]
