...
execute unless entity @e[tag=rr_pve,limit=1,type=armor_stand] run tellraw @s {"text":"Click here to disable the PVE Mode datapack (recommended).","color":"red","underline":true,"clickEvent":{"action":"run_command","value":"/datapack disable \"file/rr_pve\""}}
execute unless entity @e[tag=rr_pve,limit=1,type=armor_stand] run tellraw @s {"text":"Click here to disable the Bots datapack (recommended).","color":"red","underline":true,"clickEvent":{"action":"run_command","value":"/datapack disable \"file/rr_bots\""}}
