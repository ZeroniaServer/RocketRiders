...
execute if predicate game:game_started run function rr_sandbox:game/ingame
...
execute if entity @e[x=0,type=marker,tag=PlacerClear,tag=Cleared] unless predicate game:game_started run tag @s add sandboxLast
...
execute if entity @s[tag=EditedSettings] run tellraw @a[x=0,tag=informMe] ["",{"text":"|","bold":true,"color":"dark_gray"},{"text":" Gamemode: ","color":"#ca00ca"},{"text":"Sandbox","color":"light_purple","hoverEvent":{"action":"show_text","contents":["",{"text":"Objective:","color":"gold"},{"text":" Mess around to your heart's content!\n","color":"yellow"},{"text":"- Use the "},{"text":"Rocket","color":"blue","bold":true,"italic":false},{"text":"-","color":"gray","bold":true,"italic":false},{"text":"nomicon","color":"gold","bold":true,"italic":false},{"text":" to obtain every item in the game\n"},{"text":"- Items can stack and there is no hotbar limit\n"},{"text":"- The game still ends when the portals break\n"},{"text":"- Only one player needs to join!\n"},{"text":"Items:","color":"aqua"},{"text":" Every. Single. One.","italic":true}]}},{"text":" (hover name for info)","italic":true,"color":"dark_gray"}]
