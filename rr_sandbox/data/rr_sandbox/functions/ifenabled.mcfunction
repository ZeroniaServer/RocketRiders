#management
tag @e[type=armor_stand,tag=rr_sandbox,limit=1] add enabled
...
execute if entity @e[type=marker,tag=PlacerClear,tag=Cleared] if entity @s[tag=!GameStarted] run tellraw @a ["",{"text":"|","bold":true,"color":"dark_gray"},{"text":" Selected Gamemode: ","color":"gray"},{"text":"Normal","color":"light_purple","hoverEvent":{"action":"show_text","contents":["",{"text":"In Sandbox Mode, players on each team receive items every so often to help them construct missiles and fly them out to the enemy base. The goal is still to destroy the enemy base and portal while protecting your own, but this time, you do it with your own missiles. (Note that players may not ride their missiles in this mode in order to prevent bridging.)","color":"white"}]}},{"text":" (hover name for info)","italic":true,"color":"dark_gray"}]
...
execute if entity @s[tag=EditedSettings] run tellraw @a[tag=informMe] ["",{"text":"|","bold":true,"color":"dark_gray"},{"text":" Selected Gamemode: ","color":"gray"},{"text":"Normal","color":"light_purple","hoverEvent":{"action":"show_text","contents":["",{"text":"In Sandbox Mode, players on each team receive items every so often to help them construct missiles and fly them out to the enemy base. The goal is still to destroy the enemy base and portal while protecting your own, but this time, you do it with your own missiles. (Note that players may not ride their missiles in this mode in order to prevent bridging.)","color":"white"}]}},{"text":" (hover name for info)","italic":true,"color":"dark_gray"}]
