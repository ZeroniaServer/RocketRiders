...
execute if entity @e[predicate=custom:indimension,type=marker,tag=PlacerClear,tag=Cleared] if entity @s[tag=!GameStarted] run tellraw @a ["",{"text":"|","bold":true,"color":"dark_gray"},{"text":" Selected Gamemode: ","color":"gray"},{"text":"Normal","color":"light_purple","hoverEvent":{"action":"show_text","contents":["",{"text":"In Sandbox Mode, players receive Piercing Pickaxes and a special Rocket-nomicon book which gives them access to every item from Rocket Riders. This is just a practice gamemode to let players explore all that Rocket Riders has to offer in a non-competitive setting.","color":"white"}]}},{"text":" (hover name for info)","italic":true,"color":"dark_gray"}]
