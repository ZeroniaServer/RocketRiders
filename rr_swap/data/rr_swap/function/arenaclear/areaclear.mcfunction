...
execute unless predicate game:modifiers/hardcore/on if entity @e[x=0,type=armor_stand,tag=Selection,limit=1,tag=!swapEnabled] run fill -89 201 62 -101 195 63 cyan_stained_glass
execute unless predicate game:modifiers/hardcore/on if entity @e[x=0,type=armor_stand,tag=Selection,limit=1,tag=!swapEnabled] run fill -89 201 65 -101 195 64 blue_stained_glass
