...
execute if predicate game:phase/match/play run scoreboard players add @e[x=0,type=armor_stand,tag=Selection,limit=1] ShowTip 1
execute unless predicate game:phase/match/play run scoreboard players reset @e[x=0,type=armor_stand,tag=Selection,limit=1] ShowTip
...
execute unless predicate game:achievements_can_be_awarded as @e[x=0,type=marker,tag=SelectedTip,tag=Tip9] as @a[x=0,tag=!hideTips] run tellraw @s ["",{"text":"<","color":"dark_gray"},{"text":"TIP!","bold":true,"color":"green"},{"text":"> ","color":"dark_gray"},{"text":"There are four Special missiles which you can play with in Swap Mode!","color":"gray"}]
