execute unless entity @e[predicate=custom:indimension,type=armor_stand,tag=rr_swap,limit=1] run summon armor_stand 25 184 -6 {Tags:["rr_swap","gamemodeAS"],Marker:1b,Invisible:1b,Invulnerable:1b,CustomNameVisible:0b,CustomName:'{"text":"Swap Mode"}'}
...
execute if entity @e[predicate=custom:indimension,type=armor_stand,tag=rr_swap,limit=1] run tellraw @s {"text":"Swap Mode installed.","color":"green","bold":true}
scoreboard players add @e[predicate=custom:indimension,type=armor_stand,tag=rr_swap,limit=1] CmdData 1
